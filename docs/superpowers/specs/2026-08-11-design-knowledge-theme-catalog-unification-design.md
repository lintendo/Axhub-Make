...
设计 Tab 的侧栏资源树在确实没有任何设计规范且没有搜索词时，不再显示“暂无内容”，只替换为一句纯占位文案：`创建设计规范，统一原型的视觉与文案风格`。不拆分标题和说明，不增加点击行为或额外布局；右上角现有 `+` 继续作为唯一创建入口。输入搜索词后没有匹配结果时继续使用普通无结果状态，原型和资源 Tab 的空状态不变。
...
- 断言设计 Tab 真正为空且无搜索词时显示单句“创建设计规范，统一原型的视觉与文案风格”，占位不包含点击处理或标题/说明分层；原型、资源和搜索无结果状态不复用该文案。
