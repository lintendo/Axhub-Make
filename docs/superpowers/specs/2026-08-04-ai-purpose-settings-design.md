...
使用一张紧凑表格固定展示三行配置，列为“用途 / Agent / 模型”：

| 用途 | 覆盖入口 | Agent | 模型 |
| --- | --- | --- | --- |
| 对话 AI | 新建原型、资源、设计的初始页；右侧 ACP 对话面板 | 已安装 Agent 下拉 | 可选文本输入 |
| 批注 AI | 单条和批量批注执行；原型评审 | 已安装 Agent 下拉 | 可选文本输入 |
| 画布 AI | 根据当前画布生成；其他画布内 AI 生成和提示词优化 | 已安装 Agent 下拉 | 可选文本输入 |

模型输入留空时使用 Agent 或 ACP 自己的默认模型。未选择 Agent 时禁用对应模型输入，避免保存孤立模型值。

批注并发数保留在表格下方，继续只影响批量批注执行，不放入 Agent 检测区。
...
- 桌面宽度使用四列表格；窄屏时每个用途行改为两列标签布局，Agent 和模型输入不发生水平溢出。
