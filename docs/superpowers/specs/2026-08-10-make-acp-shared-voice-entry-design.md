...
Axhub Make 的批注语音助手首次使用时默认不显示，只能由用户在批注工具栏的“更多”菜单中显式显示；用户选择显示后，浏览器会持久化该入口偏好，使它在刷新或切换页面后继续显示。显示后的语音控件、连接状态、启动/中断交互、缺少配置时的阻断和提示统一由本地 `@axhub/acp` 组件实现，Make 不再定义独立语音面板或复制 ACP 的交互状态机。
...
- 未显示时文案为“显示语音助手”。
- 已显示时文案为“隐藏语音助手”。
- 首次使用时初始值为隐藏；用户显示或隐藏入口后，通过浏览器 `localStorage` 保存全局偏好。
- 退出批注、切换离开有效预览或关闭该菜单项时立即卸载 ACP 语音组件。
- 退出批注或切换页面只卸载当前语音组件，不清除入口显示偏好；返回有效预览后恢复悬浮入口，但不自动连接或申请麦克风权限。
...
5. Make 打开现有“设置 > AI”，直接展开并滚动到“豆包语音 API”配置区域，不增加新的 UI 面板，也不让用户从 AI 设置顶部自行查找。
...
- 在批注“更多”菜单维护显示/隐藏状态，并通过浏览器本地存储持久化该入口偏好。
...
- 刷新、切换资源或重新进入有效预览后恢复用户保存的入口显示偏好。
- 配置检查只读取脱敏接口，App ID 或 Access Key 缺失时返回未就绪。
- 设置引导打开现有 AI 设置，直接展开并滚动到“豆包语音 API”，不创建新的设置面板。
