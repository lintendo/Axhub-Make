...
export function createCompanion(config, {
  listTargets = listCursorTargets,
  attachTarget = attachCursorTarget,
...
async function runCli() {
  ...
  const launcherSource = await fs.readFile(
    path.join(path.dirname(configPath), 'axhub-make.cursor-launcher.js'),
    'utf8',
  );
  const companion = createCompanion(config, { launcherSource });
