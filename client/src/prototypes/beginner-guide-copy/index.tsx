...
type OverviewProtoState = {
    external_tool_flow_state?: 'normal' | 'optional';
};
...
function normalizeExternalToolFlowState(value: unknown): Required<OverviewProtoState>['external_tool_flow_state'] {
    return value === 'normal' || value === 'optional' ? value : 'optional';
}
...
function OverviewChapter() {
    const protoState = useProtoDevState<OverviewProtoState>();
    const externalToolFlowState = normalizeExternalToolFlowState(protoState.external_tool_flow_state);
    const isExternalToolOptional = externalToolFlowState === 'optional';
        ...
        <>
            ...
            <section className="beginner-guide-manuscript" aria-labelledby="overview-lifecycle-title">
                ...
                <div className="beginner-guide-lifecycle-flow" aria-label="原型生命周期流程">
                    ...
                    <ArrowRight className="beginner-guide-lifecycle-arrow" size={17} aria-hidden="true" />
                    <div
                        className={`beginner-guide-lifecycle-node${isExternalToolOptional ? ' is-optional' : ''}`}
                        data-annotation-id="external-tool-flow-state"
                        data-flow-state={externalToolFlowState}
                    >
                        <small>{isExternalToolOptional ? '可选流程' : '正常流程'}</small>
