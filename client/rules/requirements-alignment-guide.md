...
- 需求与设计完成第一轮对齐后，再询问用户选择 Markdown（节省 Token）或 HTML（体验更好），建议 HTML；确认后从 `templates/prototype-spec.html` 或 `templates/prototype-spec.md` 创建对应固定路径。文件存在即表示格式已选定。
...
Make 管理端默认使用 `http://localhost:53817/`；`check-app-ready` 返回 `serverUrl` 时以实际返回值为准。链接需要项目上下文时，项目 id 从 `.axhub/make/client.json` 读取。
...
优先读取上下文，已明确的信息不重复问；不能用假设代替会影响产品方向的问题。需求信息足够完整后，再进入设计方案对齐。
...
- 先读上下文，已明确的信息不重复问；不能用推断代替关键问题。
- 至少提出 3-5 个会影响产品方向的问题；一次可以询问一个或多个问题，优先合并为一条简洁的问题清单。
