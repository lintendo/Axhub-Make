...
function buildMarkdownFileUrl(projectId, markdownPath) {
    const params = new URLSearchParams({ projectId, path: markdownPath });
    return `/api/markdown-file?${params.toString()}`;
}
function findProjectDocByRouteName(projectRoot, routeName) {
...
function sendLegacyDocRedirect(res, location) {
    res.statusCode = 302;
...
function handleMarkdownFileApi(req, res, activeProjectRoot, pathname, url) {
...
function handleLegacyDocsPreview(req, res, options, projectContext, pathname, url, handlers) {
...
export function handleLegacyDocsApi(req, res, options, projectContext, pathname, url, handlers) {
