...
import { getConfigPath, maskVoiceAssistantSettings, readVoiceAssistantSettings, resolveCodexLocalImageGenerationConfig, writeVoiceAssistantSettingsPatch, } from './projectCore/index.ts';
...
export function readMakeServerVersion() {
...
function normalizeProjectServerConfig(server, availableLANHosts) {
...
function normalizeImageTestBaseUrl(value, fallback) {
...
function buildImageGenerationTestUrl(baseUrl) {
    return `${baseUrl.replace(/\/+$/u, '')}/images/generations`;
}
function createImageConfigTestSignal(timeoutMs) {
...
function readImageProviderError(body) {
...
async function readImageApiJson(response) {
...
export async function testAiImageGenerationConfig(params) {
    ...
    const baseUrl = normalizeImageTestBaseUrl(hasOwn(body, 'baseUrl') ? body.baseUrl : undefined, fallback.baseUrl);
    ...
    if (!response.ok) {
        ...
        throw new AiImageConfigTestError(providerMessage
...
export function handleConfigApi(req, res, options, pathname, context, handlers) {
    ...
    if (pathname === '/api/config/voice-assistant') {
        ...
        if (req.method === 'GET') {
            try {
                sendJson(res, {
                    settings: maskVoiceAssistantSettings(readVoiceAssistantSettings(settingsOptions)),
                });
            }
            catch (error) {
    ...
    if (pathname === '/api/config') {
        ...
        if (req.method === 'POST') {
            readJsonBody(req).then((body) => {
                ...
                const hasProjectConfigFields = Boolean(nextConfig.server
                ...
                if (hasProjectConfigFields) {
                    ...
                    if ('port' in projectConfig.server) {
                        delete projectConfig.server.port;
                    }
                    if ('allowLAN' in projectConfig.server) {
                        delete projectConfig.server.allowLAN;
                    }
                    if (nextConfig.projectInfo && typeof nextConfig.projectInfo === 'object') {
                        const nextProjectName = handlers.stringValue(nextConfig.projectInfo.name);
                        const nextProjectInfo = { ...nextConfig.projectInfo };
                    ...
                    syncProjectAgentInstructions({
                        ...
                        projectDescription: projectConfig.projectInfo?.description,
                        defaultThemeName: projectConfig.projectDefaults?.defaultTheme,
                    });
                }
                sendJson(res, {
                    success: true,
