...
import { createProjectCommunicationStore, getProjectMetadataPath, isPathInside, } from './projectCore/index.ts';
...
function hasExplicitThemeLocalPath(theme) {
    return ['sourcePath', 'path', 'filePath', 'absoluteFilePath'].some((key) => {
        const value = theme[key];
        return typeof value === 'string' && value.trim().length > 0;
    });
}
function collectLocalThemeDirectories(projectRoot, themesDir) {
...
function reconcilePrototypeFilesystemFields(prototype, scannedPrototype) {
    const fields = ['filePath', 'absoluteFilePath', 'specFilePath', 'previewDisabled'];
    if (fields.every((field) => prototype[field] === scannedPrototype[field])) {
        return prototype;
    }
    const { filePath: _filePath, absoluteFilePath: _absoluteFilePath, specFilePath: _specFilePath, previewDisabled: _previewDisabled, ...rest } = prototype;
    return {
        ...rest,
 ...
 */
function reconcileMetadataWithFilesystem(metadataStore, projectRoot) {
    ...
    const scannedPrototypeByKey = new Map();
    ...
    if (localThemeDirectories) {
        ...
        for (const [themeName, themeDir] of localThemeDirectories) {
            if (existingThemeIds.has(themeName))
                continue;
    ...
    const nextNavigationPrototypes = [];
    const seenNavigationPrototypes = new Set();
...
function readPrototypeTitle(indexFilePath, fallback) {
    try {
...
function hasGeneratedPlaceholderSource(indexFilePath) {
    try {
        ...
        return hasGeneratedShell && (source.includes('@axhub-placeholder prototype-empty')
...
function hasWaitingGenerationSource(indexFilePath) {
    try {
        const source = fs.readFileSync(indexFilePath, 'utf8');
...
function readPrototypeSpecTitle(filePath, fallback) {
    try {
...
function scanFilesystemPrototypeResources(projectRoot, prototypesDir) {
...
function scanFilesystemDocResources(projectRoot) {
    return scanResourceFiles(projectRoot);
}
function createProjectResourcesPayload(metadata, projectRoot) {
    return {
...
function createProjectNavigationPayload(metadata) {
    return {
        prototypes: metadata.navigation.prototypes,
    };
}
function createProjectOrdersPayload(metadata) {
    return {
...
function getLocalCommandErrorPayload(error) {
...
function getDocPathOutsideProjectResourceId(error) {
...
function normalizeDocContentRequestPath(value) {
...
function resolveDocContentFileByRelativePath(projectRoot, resourceId) {
...
function normalizeProjectDocumentRequestPath(value) {
    const rawValue = String(value || '').trim().replace(/\\/g, '/');
    if (!rawValue || rawValue.includes('\0') || rawValue.startsWith('/') || path.win32.isAbsolute(rawValue) || path.posix.isAbsolute(rawValue)) {
        return '';
    }
...
function resolveProjectDocumentContentFile(projectRoot, requestedPath) {
...
function normalizeProjectDocumentAssetPath(value) {
    const rawValue = String(value || '').trim().replace(/\\/g, '/');
    if (!rawValue || rawValue.includes('\0') || rawValue.startsWith('/') || path.win32.isAbsolute(rawValue) || path.posix.isAbsolute(rawValue)) {
        return '';
    }
...
function getProjectDocumentPlaceholderAssetBaseName(assetPath) {
...
function resolveProjectDocumentPlaceholderAssetFile(projectRoot, doc, requestedAssetPath) {
...
function resolveProjectDocumentAssetFile(projectRoot, requestedDocPath, requestedAssetPath) {
...
function sendProjectMetadataError(res, error, context = {}) {
...
function readProjectMetadataOrSendError(res, metadataStore, context) {
    try {
...
export function handleProjectRegistryApi(req, res, options, pathname, handlers) {
    ...
    if (rest === 'resources') {
        ...
        if (req.method === 'GET') {
            let metadata;
            try {
                metadata = backfillMakeClientResourcePreviewLinks(reconcileMetadataWithFilesystem(metadataStore, project.root), project.root, options.runtimeOrigin, req);
            }
            catch (error) {
    ...
    if (rest === 'document-asset') {
        ...
        const targetPath = resolveProjectDocumentAssetFile(project.root, url.searchParams.get('path') || '', url.searchParams.get('asset') || '');
        if (!targetPath) {
            sendJson(res, {
