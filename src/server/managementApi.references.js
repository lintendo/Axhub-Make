...
export function handleUploadAndReferenceApis(req, res, options, pathname, handlers) {
    if (pathname === '/api/markdown-file-asset' && req.method === 'GET') {
        ...
        try {
            const targetPath = handlers.resolveMarkdownFileAssetPath(context, url.searchParams.get('path') || '', url.searchParams.get('asset') || '');
            if (!targetPath) {
