...
import { buildMakeAgentSurfaceConfig, buildMakeAgentSurfaceOpenOptions, resolveMakeAgentSurfaceHost, } from './agentSurfaceIntegration.ts';
...
describe('Make agent surface integration', () => {
    ...
    it.each(['chatgpt', 'cursor', 'workbuddy', 'traework', 'qoderwork'])('injects %s without activating the current page', (provider) => {
    ...
    it.each(['chatgpt', 'cursor', 'workbuddy', 'traework', 'qoderwork'])('builds one %s project-and-surface call with an explicit application path', (provider) => {
