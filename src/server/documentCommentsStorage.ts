...
export function normalizeDocumentCommentPath(value: unknown): string | null {
...
export function documentCommentHash(documentPath: string): string {
  const normalized = normalizeDocumentCommentPath(documentPath);
  if (!normalized) throw new Error('Invalid document comment path');
  return crypto.createHash('sha256')
    .update(`document-comments:v1\0${normalized}`)
    .digest('hex');
}

export function normalizePrototypeCommentTargetPath(value: unknown): string | null {
...
export function prototypeCommentHash(targetPath: string): string {
  const normalized = normalizePrototypeCommentTargetPath(targetPath);
  if (!normalized) throw new Error('Invalid prototype comment target path');
  return crypto.createHash('sha256')
...
function validateExistingDirectory(root: string, directory: string): boolean {
...
function validateExistingFile(root: string, filePath: string): boolean {
...
export function resolveDocumentCommentStorage(
  projectRoot: string,
  documentPath: unknown,
): DocumentCommentStorage | null {
  const normalized = normalizeDocumentCommentPath(documentPath);
  ...
  if (!isInside(path.resolve(projectRoot), sourcePath)) return null;
  if (fs.existsSync(sourcePath)) {
    ...
    if (!validateExistingDirectory(realProjectRoot, parent)) return null;
  }
...
export function resolvePrototypeCommentStorage(
  ...
  if (
    !isInside(path.resolve(projectRoot), prototypeDir)
