...
function prototypeCommentHash(targetPath = 'prototypes/home') {
    return crypto.createHash('sha256')
        .update(`prototype-comments:v1\0${targetPath}`)
        .digest('hex');
}
function prototypeCommentStorage(projectRoot) {
...
function prototypeCommentAssetPath(fileName) {
    return `.axhub/make/comment-assets/${prototypeCommentHash()}/${fileName}`;
}
function writePrototypeProject(projectRoot) {
...
describe('prototype comments API', () => {
    it('returns exists:false when the prototype comments file is missing', async () => {
        ...
        try {
            const response = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/prototype-comments?targetPath=prototypes/home`));
    ...
    it('writes and reads prototype comments under the shared .axhub storage', async () => {
        ...
        const server = await startActivatedProjectServer(projectRoot);
        try {
            const put = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/prototype-comments?targetPath=prototypes/home`), {
                method: 'PUT',
            ...
            const get = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/prototype-comments?targetPath=prototypes/home`));
    ...
    it('merges schema 3 comments by durable id instead of DOM lifecycle identity', async () => {
        ...
        const server = await startActivatedProjectServer(projectRoot);
        try {
            const url = scopeProjectApiUrl(projectRoot, `${server.origin}/api/prototype-comments?targetPath=prototypes/home`);
            const write = async (comments) => {
            ...
            expect(distinct.comments.map((comment) => comment.id).sort())
    ...
    it('preserves unrelated active records added before a non-authoritative write', async () => {
        ...
        try {
            ...
            expect(response.status).toBe(200);
            const persisted = JSON.parse(fs.readFileSync(filePath, 'utf8'));
            expect(persisted.comments.map((comment) => comment.id)).toEqual([
                'comment-live',
                'comment-late',
            ]);
            ...
            expect(persisted.images.map((image) => image.id)).toEqual([
                'live-image',
                'late-image',
            ]);
        }
    ...
    it('removes only newly unreferenced image assets after an authoritative restore', async () => {
        ...
        try {
            ...
            const persisted = JSON.parse(fs.readFileSync(filePath, 'utf8'));
            expect(persisted.comments.map((comment) => comment.id)).toEqual(['comment-live', 'comment-late']);
            expect(persisted.images.map((image) => image.id)).toEqual([
                'live-shared',
                'live-only',
                'late-only',
    ...
    it('does not follow an asset-directory symlink while removing compacted images', async () => {
        ...
        const server = await startActivatedProjectServer(projectRoot);
        try {
            const escapedAssetResponse = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/prototype-comments/asset?targetPath=prototypes/home&asset=${encodeURIComponent(prototypeCommentAssetPath('linked/escape.png'))}`));
            expect(escapedAssetResponse.status).toBe(403);
            const hydratedResponse = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/prototype-comments?targetPath=prototypes/home&hydrateImages=1`));
    ...
    it('rejects non-prototype and escaped target paths', async () => {
        ...
        const server = await startActivatedProjectServer(projectRoot);
        try {
            const nonPrototype = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/prototype-comments?targetPath=components/home`));
            expect(nonPrototype.status).toBe(400);
            const escaped = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/prototype-comments?targetPath=prototypes/../home`));
            expect(escaped.status).toBe(403);
            const hidden = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/prototype-comments?targetPath=prototypes/.hidden`));
    ...
    it('extracts image payloads into shared .axhub assets and keeps base64 out of JSON', async () => {
        ...
        try {
            ...
            const assetResponse = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/prototype-comments/asset?targetPath=prototypes/home&asset=${encodeURIComponent(prototypeCommentAssetPath('hero-image.png'))}`));
    ...
    it('rejects unsafe asset paths', async () => {
        ...
        const server = await startActivatedProjectServer(projectRoot);
        try {
            const escaped = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/prototype-comments/asset?targetPath=prototypes/home&asset=${encodeURIComponent('../secret.png')}`));
            expect(escaped.status).toBe(403);
            const hidden = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/prototype-comments/asset?targetPath=prototypes/home&asset=${encodeURIComponent('.secret/image.png')}`));
            expect(hidden.status).toBe(400);
        }
        finally {
            await server.close();
