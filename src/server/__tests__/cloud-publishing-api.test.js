...
import { getConfigPath, getGlobalServerConfigPath, getMakeClientMarkerPath, getProjectExportsDir, getProjectMetadataPath, getProjectRegistryPath, } from '../projectCore/index.ts';
...
function getTestMakeHome(projectRoot) {
    return path.join(projectRoot, '.test-make-home');
}
function getTestServerConfigPath(projectRoot) {
    return getGlobalServerConfigPath(getTestMakeHome(projectRoot));
}
function writeCloudConfig(projectRoot, cloudPublishing) {
    writeJson(getTestServerConfigPath(projectRoot), {
        cloudPublishing,
    });
}
async function startTestServer(projectRoot, extraOptions = {}) {
...
function cloudflarePagesAssetHash(body, extension) {
...
function mockGitHubPagesRest(input, init) {
    const url = String(input);
    ...
    if (method === 'POST' && url.endsWith('/repos/lintendo/axhub-pages-demo/git/trees')) {
        ...
        expect(payload.tree.map((entry) => entry.path)).toEqual(expect.arrayContaining([
            'index.html',
            'index.js',
        ]));
...
function mockGitHubPagesRestWithResourcePrefix(input, init) {
    const url = String(input);
    const method = String(init?.method || 'GET').toUpperCase();
    const headers = new Headers(init?.headers);
    expect(headers.get('authorization')).toBe('Bearer gh-test-token');
    ...
    if (method === 'POST' && url.endsWith('/repos/lintendo/axhub-pages-demo/git/trees')) {
        ...
        expect(payload.tree.map((entry) => entry.path)).not.toEqual(expect.arrayContaining([
            'index.html',
            'index.js',
        ]));
        return jsonResponse({ sha: 'tree-sha' }, 201);
...
function writeCloudPublishRecord(projectRoot, input) {
...
describe('cloud publishing API', () => {
    ...
    it('defaults cloud publishing to exclude source files from published static assets', async () => {
        ...
        try {
            ...
            expect(payload.files.map((file) => file.file)).not.toEqual(expect.arrayContaining([
                'source/index.tsx',
                'source/components/Card.tsx',
            ]));
        }
    ...
    it('can enable source files for cloud publishing', async () => {
        ...
        try {
            ...
            const [, requestInit] = fetchMock.mock.calls.find(([requestUrl]) => String(requestUrl).includes('api.vercel.com')) || [];
            const payload = JSON.parse(String(requestInit?.body));
            expect(payload.files.map((file) => file.file)).toEqual(expect.arrayContaining([
                'source/index.tsx',
                'source/components/Card.tsx',
            ]));
        }
    ...
    it('injects review submit globals into cloud publishing files when the prototype config enables it', async () => {
        ...
        try {
            ...
            const [, requestInit] = fetchMock.mock.calls.find(([requestUrl]) => String(requestUrl).includes('api.vercel.com')) || [];
            const payload = JSON.parse(String(requestInit?.body));
            const htmlFile = payload.files.find((file) => file.file === 'index.html');
    ...
    it('publishes to Vercel with production target and records the export', async () => {
        ...
        try {
            ...
            expect(payload.files.map((file) => file.file)).toEqual(expect.arrayContaining(['index.html', 'index.js']));
    ...
    it('publishes to Cloudflare Pages create deployment API and reports missing project errors clearly', async () => {
        ...
        try {
            ...
            const manifest = JSON.parse(String((deploymentInit?.body).get('manifest')));
    ...
    it('uploads S3 files with SigV4 authorization and returns the configured base URL', async () => {
        ...
        try {
            ...
            expect((requestInit?.headers).Authorization).toContain('AWS4-HMAC-SHA256 Credential=AKIA_TEST/');
        }
        finally {
            await server.close();
        }
    ...
    it('auto-generates an S3 prefix from the resource path when no prefix is configured', async () => {
        ...
        try {
            ...
            expect((requestInit?.headers).Authorization).toContain('Credential=OSS_TEST/');
        }
        finally {
            await server.close();
        }
