...
function writeProjectConfig(projectRoot, value) {
...
async function startRedirectingAssistantServer(redirectLocation) {
    const server = createServer((req, res) => {
        if (req.url === '/') {
...
function writeProjectMetadata(projectRoot, id = 'assistant-client') {
...
function writeMakeClientMarker(projectRoot, id, name = 'Assistant Client') {
...
function writeMakeClientPackage(projectRoot) {
...
function writeAcpUiCheckout(root) {
...
function expectAcpUiCorsArg(args, makeOrigin) {
...
function createLocalCommandResult(command, args, stdout = '', stderr = '') {
    return {
        stdout,
        stderr,
        command,
...
beforeEach(() => {
    ...
    runLocalCommandMock.mockImplementation(async (command, args) => (createLocalCommandResult(command, args)));
});
afterEach(() => {
    vi.clearAllMocks();
...
describe('make-server assistant runtime API', () => {
    ...
    it('auto-starts the local ACP UI checkout when explicitly requested and re-probes configured endpoints', async () => {
        ...
        try {
            ...
            const spawnCall = childProcessMock.spawn.mock.calls.at(-1);
    ...
    it('auto-discovers a sibling ACP UI checkout in development before falling back to npx', async () => {
        ...
        try {
            ...
            expect(childProcessMock.spawn).not.toHaveBeenCalledWith('npx', expect.any(Array), expect.any(Object));
        }
        finally {
            await server.close();
        }
    ...
    it('does not restart a responsive configured ACP endpoint when CORS is missing', async () => {
        ...
        childProcessMock.spawnSync.mockImplementation((command, args) => {
    ...
    it('does not restart a responsive default ACP endpoint when CORS is missing', async () => {
        ...
        const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation((input, init) => {
            ...
            if (requestUrl === 'http://localhost:32124/api/chat' && method === 'OPTIONS') {
                return Promise.resolve(new Response(childProcessMock.spawn.mock.calls.length > 0 ? null : '', {
                    status: childProcessMock.spawn.mock.calls.length > 0 ? 204 : 404,
                    headers,
                }));
            }
    ...
    it('auto-start releases and restarts the default ACP port when an unreachable process is listening', async () => {
        ...
        try {
            ...
            expect(childProcessMock.spawn).not.toHaveBeenCalledWith('npx', expect.arrayContaining(['32125']), expect.any(Object));
        }
        finally {
            fetchSpy.mockRestore();
            killSpy.mockRestore();
    ...
    it('auto-starts ACP UI through npx when explicitly requested and re-probes endpoints', async () => {
        ...
        try {
            ...
            expect(commandExistsMock).toHaveBeenCalledWith('npx', expect.objectContaining({ timeoutMs: expect.any(Number) }));
            expect(runLocalCommandMock).not.toHaveBeenCalled();
            expect(childProcessMock.spawnSync).not.toHaveBeenCalled();
        }
        finally {
    ...
    it('does not persist a fallback port when 32124 is occupied by an unreachable process', async () => {
        ...
        try {
            ...
            expect(childProcessMock.spawn).not.toHaveBeenCalledWith('npx', expect.arrayContaining(['32125']), expect.any(Object));
            expect(childProcessMock.spawnSync).not.toHaveBeenCalledWith('taskkill.exe', expect.any(Array), expect.any(Object));
        }
        finally {
            killSpy.mockRestore();
            fetchSpy.mockRestore();
