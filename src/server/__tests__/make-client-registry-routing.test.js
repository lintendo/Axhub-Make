...
import { NPM_MIRROR_REGISTRY, NPM_OFFICIAL_REGISTRY, chooseNpmRegistry, deriveRegistryProbePackages, isRetryableRegistryError, probeNpmRegistry, registryInstallArgs, resolveMakeClientRegistryRoute, } from '../makeClientRegistryRouting.ts';
describe('make client npm registry routing', () => {
    ...
    it('preserves a non-default effective npm registry without probing', async () => {
        ...
        expect(runCommand).toHaveBeenCalledWith('npm', ['config', 'get', 'registry'], expect.objectContaining({ cwd: '/tmp/make-client' }));
        expect(probeRegistry).not.toHaveBeenCalled();
    });
    ...
    it('probes npmjs and npmmirror when npm uses its default registry', async () => {
        ...
        const probeRegistry = vi.fn(async (registry) => ({
            ...registry,
    ...
    it('checks ping and required package metadata during a registry probe', async () => {
        ...
        await expect(probeNpmRegistry(NPM_MIRROR_REGISTRY, [{ name: 'vite', version: '5.4.21' }], { fetchImpl, timeoutMs: 100 })).resolves.toMatchObject({ id: 'npmmirror', ok: true });
