...
import { cleanupProjectApiTestRoots, createTempRoot, initGitRepo, registerProject, scopeProjectApiUrl, startTestServer, writeProjectMetadata, } from './projects-api.helpers';
import { handleGitApi } from '../managementApi.git.ts';
const GIT_INTEGRATION_TIMEOUT_MS = 15_000;
async function commitAll(projectRoot, message) {
...
describe('make-server project git APIs', () => {
    ...
    it('builds historical prototype previews with files imported from outside the prototype directory', async () => {
        ...
        fs.writeFileSync(path.join(prototypeDir, 'index.tsx'), 'import { Badge } from "../../../src/shared/Badge";\nexport default function Home() { return Badge(); }\n', 'utf8');
        writeProjectMetadata(projectRoot, {
    ...
    it('decodes git blob stderr as text when building a historical preview fails', async () => {
        ...
        await initGitRepo(projectRoot);
        const commandExecutor = vi.fn(async (command, args) => {
            if (command !== 'git')
                throw new Error(command);
            if (args.join(' ') === '--version')
                return { stdout: 'git version 2.44.0', stderr: '' };
            ...
            if (args.join(' ') === 'status --porcelain -- src/prototypes/home')
                return { stdout: '', stderr: '' };
            if (args[0] === 'log')
                return { stdout: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa|Test User|test@example.com|1700000000|initial', stderr: '' };
            if (args[0] === 'cat-file')
                return { stdout: '', stderr: '' };
    ...
    it('reports workspace git status with user-friendly change groups', async () => {
        ...
        try {
            ...
            const otherGroup = status.body.changeSummary.groups.find((group) => group.key === 'other');
    ...
    it('automatically detects the origin remote when Make metadata is missing', async () => {
        ...
        await new Promise((resolve, reject) => {
            execFile('git', ['remote', 'add', 'origin', 'git@gitee.com:axhub/workspace-detect-origin.git'], { cwd: projectRoot }, (error, stdout, stderr) => {
                if (error) {
                    reject(new Error(String(stderr || stdout || error.message)));
                    return;
                }
    ...
    it('reports online-only and local-only workspace differences for connected remotes', async () => {
        ...
        await initGitRepo(projectRoot);
        const commandExecutor = vi.fn(async (command, args, options) => {
            if (command !== 'git')
                throw new Error(command);
            if (args.join(' ') === '--version')
                return { stdout: 'git version 2.44.0', stderr: '' };
            ...
            if (args.join(' ') === 'status --porcelain -uall')
                return { stdout: '', stderr: '' };
            if (args.join(' ') === 'branch --format=%(refname:short)')
                return { stdout: 'feature', stderr: '' };
            if (args.join(' ') === 'branch -r --format=%(refname:short)')
                return { stdout: 'origin/main\norigin/feature', stderr: '' };
            if (args.join(' ') === 'rev-parse --verify origin/main')
                return { stdout: 'origin/main', stderr: '' };
            if (args.join(' ') === 'rev-parse --verify origin/feature')
                return { stdout: 'origin/feature', stderr: '' };
            ...
            if (args.join(' ') === 'diff --name-status feature..origin/feature')
                return { stdout: '', stderr: '' };
            if (args.join(' ') === 'diff --name-status origin/feature..feature')
                return { stdout: '', stderr: '' };
    ...
    it('uses explicit refs for branch views and never executes branch mutations', async () => {
        ...
        const runGit = async (args) => (await run('git', args, projectRoot)).stdout.trim();
        ...
        const commandExecutor = vi.fn((command, args, options) => (run(command, args, options.cwd).then((result) => ({
            stdout: result.stdout.trimEnd(),
            stderr: result.stderr.trimEnd(),
        }))));
        const server = await startTestServer(projectRoot, createTempRoot('axhub-workspace-git-explicit-branch-view-home-'), { gitWorkspaceCommandExecutor: commandExecutor });
        try {
            await registerProject(server.origin, projectRoot, 'workspace-explicit-branch-view', 'Workspace Explicit Branch View');
            const status = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/git/workspace/status?branch=feature&remoteBranch=feature`)).then(async (response) => ({ status: response.status, body: await response.json() }));
            expect(status.status).toBe(200);
            expect(status.body).toMatchObject({
            ...
            expect(commandExecutor).toHaveBeenCalledWith('git', ['diff', '--name-status', 'feature..origin/feature'], { cwd: projectRoot });
            expect(commandExecutor).toHaveBeenCalledWith('git', ['diff', '--name-status', 'origin/feature..feature'], { cwd: projectRoot });
            ...
            const missingRemote = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/git/workspace/status?branch=feature&remoteBranch=missing`)).then(async (response) => ({ status: response.status, body: await response.json() }));
    ...
    it('persists the origin remote created by the repository CLI', async () => {
        ...
        const commandExecutor = vi.fn(async (command, args) => {
            if (command === 'git') {
                ...
                if (args.join(' ') === 'status --porcelain -uall')
                    return { stdout: '', stderr: '' };
                if (args.join(' ') === 'remote get-url origin' && originUrl)
                    return { stdout: originUrl, stderr: '' };
                if (args[0] === 'remote')
        ...
        try {
            ...
            expect(commandExecutor).toHaveBeenCalledWith('gh', ['repo', 'create', 'acme/workspace-created', '--private', '--source=.', '--remote=origin', '--confirm'], { cwd: projectRoot });
            const config = JSON.parse(fs.readFileSync(path.join(projectRoot, '.axhub', 'make', 'axhub.config.json'), 'utf8'));
