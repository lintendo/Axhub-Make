...
import { getAdminServerInfoPath, getGlobalAdminServerInfoPath, getGlobalMakeStateDir, getConfigPath, getMakeClientMarkerPath, getRuntimeServerInfoPath, getProjectMetadataPath, getProjectRegistryPath, readServerInfo, SIDEBAR_TREE_STORE_RELATIVE_PATH, } from '../projectCore/index.ts';
...
function writePrototype(root, name, source = 'export default function Page() {}') {
...
function writeMakeClientProjectMarker(root, id, name) {
...
async function registerExistingMakeProject(origin, projectRoot, expectedStatus = 201) {
...
describe('make-server HTTP server', () => {
    ...
    it('discovers unique non-internal IPv4 LAN host candidates', () => {
        ...
        })).toEqual(['192.168.31.88', '10.0.8.42']);
    });
    ...
    it('serves startup GET APIs with fallback payloads when active project metadata is missing', async () => {
        ...
        try {
            ...
            expect(responses.map(({ requestPath, status }) => ({ requestPath, status }))).toEqual(requests.map((requestPath) => ({ requestPath, status: 200 })));
    ...
    it('requires explicit project IDs for startup fallback APIs when active project metadata is missing', async () => {
        ...
        try {
            ...
            const responses = await Promise.all(requests.map((requestPath) => (fetch(`${server.origin}${requestPath}`))));
    ...
    it('treats legacy websocket send as optional when the runtime route is missing', async () => {
        ...
        await new Promise((resolve) => runtimeServer.listen(0, 'localhost', resolve));
    ...
    it('serves legacy docs preview routes from make-server before runtime proxy fallback', async () => {
        ...
        fs.writeFileSync(path.join(adminRoot, 'spec-template.html'), [
    ...
    it('serves spec template HTML through Vite transforms in dev mode', async () => {
        ...
        const adminRoot = path.join(projectRoot, 'admin-dist');
        fs.mkdirSync(adminRoot, { recursive: true });
        fs.writeFileSync(path.join(adminRoot, 'spec-template.html'), '<html><head><title>{{TITLE}}</title></head><body><script type="module" src="/assets/spec-template-bootstrap.js"></script></body></html>', 'utf8');
        const transformHtml = vi.fn(async (url, htmlPath, _injectScript) => {
        ...
        try {
            ...
            expect(transformHtml).toHaveBeenCalledWith('/spec-template.html', path.join(adminRoot, 'spec-template.html'), expect.stringContaining('window.__AXHUB_MAKE_API_ORIGIN__'));
    ...
    it('serves dev-mode APIs while the first admin frontend request waits for Vite', async () => {
        ...
        try {
            ...
            const rootRace = await Promise.race([
                rootResponsePromise.then(() => 'resolved'),
                new Promise((resolve) => setTimeout(() => resolve('pending'), 25)),
            ]);
            expect(rootRace).toBe('pending');
            resolveMiddleware?.({
                handle: vi.fn(),
                transformHtml: vi.fn(async (_url, htmlPath) => fs.readFileSync(htmlPath, 'utf8')),
                close: vi.fn(),
            });
            ...
            if (req.url === '/api/health') {
                ...
                res.end(JSON.stringify({
                    ...
                    server: {
                        ...
                        host: 'localhost',
                        origin: `http://localhost:${runtimeServer.address().port}`,
                        projectRoot,
    ...
    it('proxies dev-mode runtime module requests with prototype referers before admin Vite can restrict them', async () => {
        ...
        const viteHandle = vi.fn((_req, res) => {
            res.statusCode = 403;
    ...
    it('proxies active project runtime routes through the current project runtime instead of a stale admin runtime origin', async () => {
        ...
        const activeRuntimeServer = http.createServer((req, res) => {
            if (req.url === '/api/health') {
                ...
                res.end(JSON.stringify({
                    ...
                    server: {
                        pid: process.pid,
                        port: activeRuntimeServer.address().port,
                        host: 'localhost',
                        origin: `http://localhost:${activeRuntimeServer.address().port}`,
                        projectRoot,
                        startedAt: new Date().toISOString(),
                        timestamp: new Date().toISOString(),
                    },
        ...
        await new Promise((resolve) => activeRuntimeServer.listen(0, 'localhost', resolve));
        await new Promise((resolve) => staleRuntimeServer.listen(0, 'localhost', resolve));
        const activeRuntimeAddress = activeRuntimeServer.address();
        const staleRuntimeAddress = staleRuntimeServer.address();
        ...
        try {
            ...
            await server.close();
            await new Promise((resolve) => activeRuntimeServer.close(() => resolve()));
            await new Promise((resolve) => staleRuntimeServer.close(() => resolve()));
        }
    });
    it('does not proxy an unknown project ID to the stale admin runtime origin', async () => {
        const projectRoot = createProjectRoot();
        const staleRuntimeRequests = [];
        const staleRuntimeServer = http.createServer((req, res) => {
        ...
        try {
            ...
            const registeredProjectResponse = await fetch(`${server.origin}/prototypes/annotation-demo?projectId=test-client`);
    ...
    it('keeps the configured runtime fallback after rejecting an unknown project request', async () => {
        const projectRoot = createProjectRoot();
        const configuredRuntimeRequests = [];
        ...
        await new Promise((resolve) => configuredRuntimeServer.listen(0, 'localhost', resolve));
        const configuredRuntimeAddress = configuredRuntimeServer.address();
        ...
        try {
            const unknownProjectResponse = await fetch(`${server.origin}/prototypes/annotation-demo?projectId=missing-project`);
            ...
            await server.close();
            await new Promise((resolve) => configuredRuntimeServer.close(() => resolve()));
        }
    });
    it('injects the active project runtime into the admin shell for an unknown project ID', async () => {
        ...
        const adminRoot = path.join(projectRoot, 'admin-dist');
        fs.mkdirSync(adminRoot, { recursive: true });
        fs.writeFileSync(path.join(adminRoot, 'index.html'), '<html><head></head><body>Admin</body></html>', 'utf8');
        const activeRuntimeServer = http.createServer((req, res) => {
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json; charset=utf-8');
            res.end(JSON.stringify({
    ...
    it('serves project title and sidebar navigation from make project state files', async () => {
        ...
        try {
            ...
            expect(entries.prototypes.every((item) => !Object.prototype.hasOwnProperty.call(item, 'demoUrl'))).toBe(true);
    ...
    it('uses metadata theme order for workspace theme order and flat navigation trees', async () => {
        ...
        try {
            ...
            expect(navigation.tree.map((node) => node.itemKey)).toEqual([
                'themes/apple',
                'themes/linear',
                'themes/notion',
                'themes/airbnb',
    ...
    it('normalizes generated repeated theme titles from persisted workspace navigation', async () => {
        ...
        try {
            await registerExistingMakeProject(server.origin, projectRoot);
            const navigation = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/workspace/navigation?tab=themes`)).then((response) => response.json());
            expect(navigation.tree.map((node) => node.title)).toEqual([
                'Figma',
                'Orderful',
                '手动主题名',
            ]);
            const stored = JSON.parse(fs.readFileSync(path.join(projectRoot, SIDEBAR_TREE_STORE_RELATIVE_PATH), 'utf8'));
            expect(stored.themesTree.map((node) => node.title)).toEqual([
                'Figma',
                'Orderful',
                '手动主题名',
            ]);
