...
function writePrototype(projectRoot, name, options = {}) {
...
function writeProjectMetadata(projectRoot, prototypes) {
...
function createResponse() {
    ...
    response.hasHeader = function hasHeader(name) {
    ...
    response.end = function endResponse(chunk, encodingOrCallback, callback) {
        if (chunk) {
            chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk));
        }
...
async function startAdminStaticServer(projectRoot, adminRoot) {
...
describe('make-server admin static assets', () => {
    it('redirects missing untitled prototype short links to the latest prototype and preserves the source p', () => {
        ...
        expect(handleAdminStatic(createRequest('/?projectId=make14&p=untitled-5&v=canvas'), redirect.response, { adminRoot, projectRoot, host: 'localhost', port: 5174 })).toBe(true);
    ...
    it('does not redirect existing placeholders, regular missing prototypes, or empty prototype lists', () => {
        ...
        expect(handleAdminStatic(createRequest('/?projectId=make14&p=untitled-5&v=canvas'), existing.response, { adminRoot, projectRoot, host: 'localhost', port: 5174 })).toBe(true);
        expect(existing.statusCode).toBe(200);
        expect(existing.header('location')).toBe('');
        const regularMissing = createResponse();
        expect(handleAdminStatic(createRequest('/?projectId=make14&p=normal-old-name&v=canvas'), regularMissing.response, { adminRoot, projectRoot, host: 'localhost', port: 5174 })).toBe(true);
        ...
        expect(handleAdminStatic(createRequest('/?projectId=make14&p=untitled-9&v=canvas'), empty.response, { adminRoot, projectRoot: emptyProjectRoot, host: 'localhost', port: 5174 })).toBe(true);
    ...
    it('uses prototype directory mtime before falling back to the resource list order', () => {
        ...
        const redirect = createResponse();
        expect(handleAdminStatic(createRequest('/?projectId=make14&p=untitled&v=canvas'), redirect.response, { adminRoot, projectRoot, host: 'localhost', port: 5174 })).toBe(true);
        expect(redirect.statusCode).toBe(302);
    ...
    it('serves resource canvas template HTML without Vite dev-only module imports', async () => {
        ...
        writeFile(path.join(adminRoot, 'index.html'), '<html><head></head><body>Admin</body></html>');
        writeFile(path.join(adminRoot, 'canvas-template.html'), [
            '<html><head>',
    ...
    it('serves spec template HTML without Vite dev-only module imports', async () => {
        ...
        writeFile(path.join(adminRoot, 'index.html'), '<html><head></head><body>Admin</body></html>');
        writeFile(path.join(adminRoot, 'spec-template.html'), [
    ...
    it('does not serve OpenCode WebUI static assets while the feature is disabled', async () => {
        ...
        writeFile(path.join(adminRoot, 'index.html'), '<html><head></head><body>Admin</body></html>');
        writeFile(path.join(adminRoot, 'opencode-webui/index.html'), '<html><head></head><body><div id="root">OpenCode</div></body></html>');
        ...
        try {
            ...
            expect(handleAdminStatic(createRequest('/assets/index.js', { referer: 'http://localhost/opencode/project/session' }), rootAsset.response, {
                adminRoot,
                projectRoot,
                host: 'localhost',
                port: 5174,
    ...
    it.skip('serves root OpenCode asset references from the OpenCode static root', async () => {
        ...
        writeFile(path.join(adminRoot, 'opencode-webui/index.html'), '<html><head></head><body><div id="root"></div></body></html>');
        writeFile(path.join(adminRoot, 'opencode-webui/assets/sprite.svg'), '<svg id="opencode"></svg>');
        const openCodeAsset = createResponse();
        expect(handleAdminStatic(createRequest('/assets/sprite.svg', { referer: 'http://localhost/opencode/project/session' }), openCodeAsset.response, {
            adminRoot,
            projectRoot,
            host: 'localhost',
            port: 5174,
        ...
        expect(handleAdminStatic(createRequest('/assets/sprite.svg'), adminAsset.response, {
            adminRoot,
            projectRoot,
            host: 'localhost',
            port: 5174,
        ...
        expect(handleAdminStatic(createRequest('/assets/missing.svg', { referer: 'http://localhost/opencode/project/session' }), missingOpenCodeAsset.response, {
            adminRoot,
            projectRoot,
            host: 'localhost',
            port: 5174,
    ...
    it.skip('does not fall back to OpenCode HTML for static asset misses', async () => {
        ...
        expect(handleAdminStatic(createRequest('/opencode/assets/missing.js'), missingAsset.response, {
            adminRoot,
            projectRoot,
            host: 'localhost',
            port: 5174,
        ...
        expect(handleAdminStatic(createRequest(`/opencode/assets/${encodeURIComponent('../../index.html')}`), escapedAsset.response, {
            adminRoot,
            projectRoot,
            host: 'localhost',
            port: 5174,
    ...
    it('returns false for missing templates and escaped static paths in direct handler calls', async () => {
        ...
        expect(handleAdminStatic(createRequest('/index.html'), missingIndex.response, { adminRoot, projectRoot, host: 'localhost', port: 5174 })).toBe(false);
        const missingCanvasTemplate = createResponse();
        expect(handleAdminStatic(createRequest('/canvas/resources/flows/home.excalidraw'), missingCanvasTemplate.response, { adminRoot, projectRoot, host: 'localhost', port: 5174 })).toBe(false);
        const removedPrototypeCanvas = createResponse();
        expect(handleAdminStatic(createRequest('/canvas/prototypes/home/canvas.excalidraw'), removedPrototypeCanvas.response, { adminRoot, projectRoot, host: 'localhost', port: 5174 })).toBe(false);
        const escapedHtml = createResponse();
        expect(handleAdminStatic(createRequest('/admin/../outside.html'), escapedHtml.response, { adminRoot, projectRoot, host: 'localhost', port: 5174 })).toBe(false);
        const escapedAsset = createResponse();
        expect(handleAdminStatic(createRequest('/assets/../secret.js'), escapedAsset.response, { adminRoot, projectRoot, host: 'localhost', port: 5174 })).toBe(false);
        const autoDebug = createResponse();
        expect(handleAdminStatic(createRequest('/auto-debug-client.js'), autoDebug.response, { adminRoot, projectRoot, host: 'localhost', port: 5174 })).toBe(true);
