...
import { cleanupProjectApiTestRoots, createTempRoot, createZipFromDirectory, registerProject, scopeProjectApiUrl, startTestServer, setActiveProject, writeProjectMetadata, } from './projects-api.helpers';
import { handlePrototypeUploadApi } from '../managementApi.prototypeUpload.ts';
afterEach(() => {
    vi.restoreAllMocks();
    cleanupProjectApiTestRoots();
});
function writeUploadEnabledProject(projectRoot, id) {
...
function writeConverterScript(projectRoot, scriptFile, taskFileName, extraOutputs = {}) {
    const scriptsDir = path.join(projectRoot, 'scripts');
    fs.mkdirSync(scriptsDir, { recursive: true });
    const extraWrites = Object.entries(extraOutputs)
        .map(([relativePath, content]) => (`fs.mkdirSync(path.dirname(path.join(outputDir, ${JSON.stringify(relativePath)})), { recursive: true });\n`
...
function writeAllConverterScripts(projectRoot) {
...
function writeAxureHtmlConverterScript(projectRoot) {
    const scriptsDir = path.join(projectRoot, 'scripts');
    fs.mkdirSync(scriptsDir, { recursive: true });
...
function createUnsafeZipWithTraversalEntry() {
...
function createStoredZipWithEncodedEntries(entries, zipPath) {
    const chunks = [];
    const centralChunks = [];
    let offset = 0;
...
function createLegacyChineseFileNameZip() {
...
function writePlaceholderPrototype(projectRoot, prototypeName = 'untitled') {
...
async function registerAndActivateProject(origin, projectRoot, projectId, projectName = projectId) {
...
describe('make-server project prototype upload APIs', () => {
    ...
    it('creates placeholder prototypes with clean source files and no canvas file', async () => {
        ...
        try {
            ...
            const metadata = readMetadata(projectRoot);
            const prototype = metadata.resources.prototypes.find((item) => item.id === 'untitled');
    ...
    it('moves placeholder prototypes into a client waiting state before generation starts', async () => {
        ...
        try {
            ...
            let prototype = metadata.resources.prototypes.find((item) => item.id === 'untitled');
            expect(prototype).toMatchObject({
                id: 'untitled',
                name: 'untitled',
                clientUrl: `${server.origin}/prototypes/untitled`,
            ...
            prototype = metadata.resources.prototypes.find((item) => item.id === 'untitled');
            expect(prototype).toMatchObject({
                generationStatus: 'waiting',
            });
            expect(prototype).not.toHaveProperty('placeholder');
    ...
    it('uploads zip files whose Chinese entry names use legacy GB18030 bytes', async () => {
        ...
        const server = await startTestServer(projectRoot);
        try {
            await registerAndActivateProject(server.origin, projectRoot, 'prototype-zip-legacy-chinese-client', 'Prototype Zip Legacy Chinese Client');
            const form = new FormData();
            form.append('uploadType', 'make');
            form.append('targetType', 'prototypes');
            form.append('uploadMode', 'zip');
