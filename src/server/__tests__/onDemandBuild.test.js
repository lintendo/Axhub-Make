...
function createTempProject() {
    ...
    fs.writeFileSync(path.join(root, 'src', 'entry.tsx'), [
...
function writeFakePackage(projectRoot, packageName, packageJson, content) {
...
function writeFakeBuildToolchain(projectRoot) {
    writeFakePackage(projectRoot, 'vite', { main: 'index.cjs' }, [
    ...
    writeFakePackage(projectRoot, '@vitejs/plugin-react', { type: 'module', main: 'index.js' }, 'export default function react() { return { name: "fake-react" }; }\n');
    writeFakePackage(projectRoot, '@tailwindcss/vite', { type: 'module', main: 'index.js' }, 'export default function tailwindcss() { return { name: "fake-tailwind" }; }\n');
}
function writeFakeAnnotationRuntimePackage(projectRoot) {
    writeFakePackage(projectRoot, '@axhub/annotation', { type: 'module', main: 'index.js' }, [
        'import React from "react";',
...
describe('buildOnDemand', () => {
    ...
    it('includes same-directory style.css even when the entry does not import it', async () => {
        const root = createTempProject();
        writeFile(path.join(root, 'src', 'style.css'), [
            '.entry {',
            '  color: rgb(12 34 56);',
            '}',
            '',
    ...
    it('reports when an entry bundles the annotation runtime', async () => {
        ...
        writeFile(path.join(root, 'src', 'entry.tsx'), [
            'import React from "react";',
    ...
    it('uses the annotation-source markdown inlining plugin for export builds', async () => {
        const root = createTempProject();
        writeFakePackage(root, 'vite', { main: 'index.cjs' }, [
            'module.exports.defineConfig = (config) => config;',
            'Object.assign(module.exports, { mergeConfig: (config) => config });',
        ...
        writeFakePackage(root, '@vitejs/plugin-react', { type: 'module', main: 'index.js' }, 'export default function react() { return { name: "fake-react" }; }\n');
        writeFakePackage(root, '@tailwindcss/vite', { type: 'module', main: 'index.js' }, 'export default function tailwindcss() { return { name: "fake-tailwind" }; }\n');
        const result = await buildOnDemand(root, path.join(root, 'src', 'entry.tsx'));
        expect(result.jsCode).toContain('UserComponent');
    });
...
function pathToFileImport(filePath) {
