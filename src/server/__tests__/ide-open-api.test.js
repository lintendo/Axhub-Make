...
import { getConfigPath, getGlobalServerConfigPath, getMakeClientMarkerPath, getProjectMetadataPath, getProjectRegistryPath, } from '../projectCore/index.ts';
...
function writeProjectMetadata(projectRoot) {
...
afterEach(() => {
    ...
    childProcessMock.spawn.mockImplementation((_command, _args) => createSpawnOnlyMockChild());
...
describe('make-server IDE open API', () => {
    ...
    it('opens Unix IDEs with the platform open command and reports stderr on failure', async () => {
        ...
        expect(childProcessMock.spawn).toHaveBeenCalledWith('open', ['-a', 'Visual Studio Code', '/workspace/demo/Axhub Runtime'], expect.objectContaining({
            detached: true,
            shell: false,
        }));
    ...
    it('opens Windows IDEs through executable discovery and app-path fallback', async () => {
        ...
        try {
            ...
            expect(childProcessMock.spawn).toHaveBeenCalledWith('C:\\Tools\\cursor.exe', ['C:\\Projects\\Axhub Runtime'], expect.objectContaining({
                detached: true,
                windowsHide: true,
                shell: false,
            }));
            ...
            expect(childProcessMock.spawn).toHaveBeenCalledWith('powershell', expect.arrayContaining([
                '-NoProfile',
                '-NonInteractive',
                '-WindowStyle',
                'Hidden',
            ...
            expect(childProcessMock.spawn).not.toHaveBeenCalledWith('powershell', expect.arrayContaining(['trae://file/C:/Projects/Fallback']), expect.anything());
        }
        finally {
    ...
    ])('returns the %s Windows file protocol for browser execution when executable launches fail', async (ide, expectedUrl) => {
        const originalPlatform = process.platform;
        Object.defineProperty(process, 'platform', { value: 'win32', configurable: true });
        childProcessMock.spawnSync.mockReturnValue({ status: 1, stdout: '', stderr: '' });
    ...
    it('uses a stored Windows IDE executable path before probing and records the direct-app open mode', async () => {
        ...
        try {
            ...
            expect(childProcessMock.spawn).toHaveBeenCalledWith('C:\\Stored\\Cursor.exe', [path.join(projectRoot, 'src/prototypes/home/index.tsx')], expect.objectContaining({
                shell: false,
                windowsHide: true,
            }));
    ...
    it('uses the stored Windows IDE browser-deeplink mode before direct executable launch', async () => {
        ...
        try {
            ...
            expect(childProcessMock.spawn).not.toHaveBeenCalledWith('C:\\Tools\\Cursor.exe', expect.anything(), expect.anything());
        }
        finally {
            Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true });
            await server.close();
    ...
    it('falls back to browser-side deeplinks when the Windows IDE command wrapper exits unsuccessfully', async () => {
        ...
        childProcessMock.spawn.mockImplementation((command) => {
            if (command === 'cmd.exe') {
    ...
    it('prefers a registered Windows file protocol over direct executable launch for browser execution', async () => {
        ...
        try {
            ...
            expect(childProcessMock.spawn).not.toHaveBeenCalledWith('C:\\Users\\demo\\AppData\\Local\\Programs\\Cursor\\Cursor.exe', expect.anything(), expect.anything());
        }
        finally {
            Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true });
        }
    ...
    it('opens a project-relative target in the configured IDE', async () => {
        ...
        try {
            ...
            expect(childProcessMock.spawn).toHaveBeenCalledWith('open', ['-a', 'Cursor', sourcePath], expect.objectContaining({
                detached: true,
                shell: false,
            }));
        }
