...
function writePrototypeProject(projectRoot, indexSource = 'export default function Home() { return <main id="hero" />; }\n') {
...
describe('prototype annotation API', () => {
    ...
    it('creates a standard page directory from valid multi-page metadata', async () => {
        ...
        try {
            ...
            expect(JSON.parse(fs.readFileSync(path.join(projectRoot, 'src/prototypes/home/annotation-source.json'), 'utf8')).directory).toEqual(body.source.directory);
        }
        finally {
            await server.close();
        }
    ...
    it('deletes an existing annotation node when writing empty markdown', async () => {
        ...
        try {
            ...
            expect(body.source.data.nodes.map((node) => node.id)).toEqual(['footer']);
            expect(body.source.markdownMap).toEqual({ footer: '页脚需求' });
            expect(persisted.data.nodes.map((node) => node.id)).toEqual(['footer']);
