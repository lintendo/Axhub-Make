...
import { applyHtmlTextEdits, clearHtmlTemporaryStyleHack, indexHtmlEditableText, injectHtmlEditingMetadata, readBoundedJson, upsertHtmlTemporaryStyleHack, } from '../htmlResourceEditing.ts';
import { cleanupProjectApiTestRoots, createTempRoot, registerProject, setActiveProject, startTestServer, writeProjectMetadata, } from './projects-api.helpers.ts';
afterEach(() => {
...
describe('HTML temporary style hacks', () => {
    ...
    it('clears the complete block and rejects unsafe CSS or stale revisions', () => {
        ...
        expect(() => upsertHtmlTemporaryStyleHack(html, revision, '/* <!-- axhub:temporary-style-hack:start --> */ p {}')).toThrowError(expect.objectContaining({ code: 'HTML_STYLE_HACK_INVALID', status: 400 }));
        expect(() => upsertHtmlTemporaryStyleHack(html, revision, '/* <!-- axhub:temporary-style-hack:end --> */ p {}')).toThrowError(expect.objectContaining({ code: 'HTML_STYLE_HACK_INVALID', status: 400 }));
    });
    it('inserts at real document boundaries instead of closing-tag text in scripts or comments', () => {
        ...
        const scriptResult = upsertHtmlTemporaryStyleHack(scriptHtml, indexHtmlEditableText(scriptHtml).revision, 'p { color: red; }');
        ...
        const commentResult = upsertHtmlTemporaryStyleHack(commentHtml, indexHtmlEditableText(commentHtml).revision, 'p { color: blue; }');
    ...
    it('rejects a style block that would grow the HTML beyond 2 MB', () => {
        ...
        expect(Buffer.byteLength(html, 'utf8')).toBeLessThan(2_000_000);
        expect(() => upsertHtmlTemporaryStyleHack(html, indexHtmlEditableText(html).revision, `p::before { content: '${'a'.repeat(20_000)}'; }`)).toThrowError(expect.objectContaining({ code: 'HTML_DOCUMENT_TOO_LARGE', status: 413 }));
    });
});
