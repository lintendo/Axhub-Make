...
import { getConfigPath, getMakeClientMarkerPath, getProjectEditHistoryDir, getProjectExportsDir, getProjectMetadataPath, getProjectRegistryPath, getProjectSessionsDir, } from '../projectCore/index.ts';
...
function writeMakeClientPackageForProject(projectRoot) {
...
function writeGeneratedPlaceholderPrototype(prototypesDir, name) {
...
async function startTestServer(projectRoot, registryHome = createTempRoot('axhub-make-projects-api-home-')) {
...
describe('make-server project APIs', () => {
    ...
    it('reconciles project resources from the filesystem when serving resources', async () => {
        ...
        try {
            ...
            const placeholderStartedPrototype = body.resources.prototypes.find((prototype) => prototype.id === 'placeholder-started');
            ...
            const readyPrototype = body.resources.prototypes.find((prototype) => prototype.id === 'ready');
            ...
            for (const id of ['explicit-path-theme', 'explicit-file-theme', 'explicit-absolute-theme']) {
                expect(body.resources.themes.find((theme) => theme.id === id)).not.toHaveProperty('sourcePath');
            }
            ...
            const storedPlaceholderStartedPrototype = stored.resources.prototypes.find((prototype) => prototype.id === 'placeholder-started');
            ...
            const storedReadyPrototype = stored.resources.prototypes.find((prototype) => prototype.id === 'ready');
    ...
    it('reads template markdown content from src/resources without metadata docs or templates indexes', async () => {
        ...
        fs.writeFileSync(path.join(projectRoot, 'src', 'resources', 'templates', 'prd-template.md'), '# PRD Template\n\nBody.\n', 'utf8');
        writeProjectMetadata(projectRoot, {
    ...
    it('serves relative assets for project-internal markdown documents', async () => {
        ...
        try {
            await registerExistingMakeProject(server.origin, projectRoot);
            const response = await fetch(`${server.origin}/api/projects/client-a/document-asset?path=${encodeURIComponent('src/prototypes/annotation-demo/docs/prd-05-handoff.md')}&asset=${encodeURIComponent('assets/handoff.png')}`);
            expect(response.status).toBe(200);
    ...
    it('serves annotation demo markdown image placeholders from prototype assets', async () => {
        ...
        try {
            await registerExistingMakeProject(server.origin, projectRoot);
            const response = await fetch(`${server.origin}/api/projects/client-a/document-asset?path=${encodeURIComponent('src/prototypes/annotation-demo/docs/prd-05-handoff.md')}&asset=${encodeURIComponent('__ANNOTATION_IMAGE_DOCUMENT_EDIT__')}`);
            expect(response.status).toBe(200);
            expect(response.headers.get('content-type')).toBe('image/png');
