...
async function startRoutingServer(projectRoot, options = {}) {
...
afterEach(async () => {
    await Promise.all(httpServers.splice(0).map((server) => new Promise((resolve, reject) => {
        server.close((error) => {
            if (error)
                reject(error);
            else
...
function openWebSocketUpgrade(origin, requestPath) {
...
describe('make-server HTTP routing', () => {
    ...
    it('uses the explicit projectId to choose the runtime origin for proxied prototype documents', async () => {
        ...
        for (const [root, projectId] of [[selectedProjectRoot, 'selected-project'], [activeProjectRoot, 'active-project']]) {
        ...
        try {
            ...
            const runtimeWithoutProject = await fetch(`${server.origin}/prototypes/touch-and-talk-annotation-demo/index.tsx`, { headers: { accept: 'application/javascript' } });
