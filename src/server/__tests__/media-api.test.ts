...
function scopeMediaApiUrl(origin: string, requestPath: string): string {
...
describe('make-server media API', () => {
  it('creates folders, serves files, and deletes assets inside the media root', async () => {
    ...
    try {
      const create = await fetch(scopeMediaApiUrl(server.origin, `/api/media/folder`), {
        method: 'POST',
      ...
      const file = await fetch(scopeMediaApiUrl(server.origin, `/api/media/file/icons/logo.svg`));
      expect(file.status).toBe(200);
      expect(file.headers.get('content-type')).toContain('image/svg+xml');
      expect(await file.text()).toBe('<svg />');

      const list = await fetch(scopeMediaApiUrl(server.origin, `/api/media?path=${encodeURIComponent('icons')}`)).then((response) => response.json());
      ...
      const deleted = await fetch(scopeMediaApiUrl(server.origin, `/api/media/icons/photos`), { method: 'DELETE' });
  ...
  it('rejects traversal, backslash escape, and root delete requests', async () => {
    ...
    try {
      const unsafeFolder = await fetch(scopeMediaApiUrl(server.origin, `/api/media/folder`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const unsafeParent = await fetch(scopeMediaApiUrl(server.origin, `/api/media/folder`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const unsafeBackslash = await fetch(scopeMediaApiUrl(server.origin, `/api/media/file/${encodeURIComponent('..\\\\outside.svg')}`));
      expect(unsafeBackslash.status).toBe(403);

      const rootDelete = await fetch(scopeMediaApiUrl(server.origin, `/api/media/`), { method: 'DELETE' });
