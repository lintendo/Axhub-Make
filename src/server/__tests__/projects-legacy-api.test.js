...
import { cleanupProjectApiTestRoots, createTempRoot, registerProject, scopeProjectApiUrl, setActiveProject, startTestServer, writeJson, writeProjectMetadata, } from './projects-api.helpers';
...
describe('make-server project legacy compatibility APIs', () => {
    ...
    it('routes legacy compatibility APIs through the explicit project context', async () => {
        ...
        try {
            ...
            expect(docs.map((doc) => doc.name)).toEqual(['second.md']);
    ...
    it('routes legacy file operations through the requested project instead of the active project', async () => {
        ...
        try {
            ...
            expect(projects.projects.map((project) => project.id)).toEqual(expect.arrayContaining(['first-client', 'second-client']));
            ...
            expect(secondMetadata.resources.prototypes.map((prototype) => prototype.id)).toEqual(['second-only']);
    ...
    it('hides resource README and dot-prefixed files from the legacy docs list', async () => {
        ...
        try {
            ...
            expect(docs.map((doc) => doc.name)).toEqual(['visible.json']);
        }
        finally {
            await server.close();
        }
    ...
    it('shows the unsupported-file preview shell for legacy markdown-file browser navigation to drawio resources', async () => {
        ...
        try {
            ...
            const response = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/markdown-file?path=${encodeURIComponent('src/resources/order-status-flow.drawio')}`), {
                headers: {
    ...
    it('injects the shared HTML annotation bootstrap into legacy markdown-file HTML previews', async () => {
        ...
        try {
            ...
            const response = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/markdown-file?path=${encodeURIComponent('src/resources/visual-prd.html')}`), {
                headers: {
                    accept: 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8',
                },
            });
