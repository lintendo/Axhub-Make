...
const { buildLocalAppOpenCommandForPlatform, buildLocalAppOpenResultForPlatform, buildLocalAppLaunchCommandForPlatform, getMissingCLIAgentOpenError, getMissingLocalAppOpenError, getMissingWebAgentOpenError, openCLIAgent, openLocalAppApplication, openLocalAppAgent, openWebAgent, readManagedOpenCodeServerUrl, } = await import('../agentOpen.ts');
...
function writeProjectMetadata(projectRoot, projectId = 'agent-client', projectName = 'Agent Client') {
...
async function startTestServer(projectRoot, options = {}) {
...
function mockDetectedCommands(commands) {
    childProcessMock.spawnSync.mockImplementation((...input) => {
        ...
        const shellCommand = command.endsWith('sh') || command.endsWith('zsh') || command.endsWith('bash')
            ...
            : '';
        const matched = commands.find((candidate) => (String(args[0] || '') === candidate
...
function mockMissingMacApplications(...applicationNames) {
...
describe('make-server agent open API', () => {
    ...
    it('routes TRAEWORK to a surface-only desktop integration operation', async () => {
        ...
        try {
            ...
            expect(coordinateDesktopIntegrationOpenMock).toHaveBeenCalledWith({ provider: 'traework', action: 'prepare' }, expect.objectContaining({ open: expect.any(Function) }));
            ...
            const firstSurfaceOpenCall = openMakeAgentSurfaceMock.mock.calls[0];
    ...
    it('launches TRAEWORK without a directory when entry injection is disabled', async () => {
        ...
        try {
            ...
            expect(response.status).toBe(200);
            expect(coordinateDesktopIntegrationOpenMock).toHaveBeenCalledWith({ provider: 'traework', action: 'normal' }, expect.objectContaining({ open: expect.any(Function) }));
            const adapters = coordinateDesktopIntegrationOpenMock.mock.calls[0]?.[1];
            ...
            expect(openMakeAgentSurfaceProjectMock).not.toHaveBeenCalled();
            expect(openMakeAgentProjectOnlyMock).not.toHaveBeenCalled();
            expect(childProcessMock.spawn).toHaveBeenCalledWith('open', ['-a', appBundle], expect.objectContaining({ shell: false }));
            const firstApplicationOpenCall = childProcessMock.spawn.mock.calls[0];
    ...
    it.each(['workbuddy', 'qoderwork'])('routes the %s iframe host through the desktop integration coordinator', async (provider) => {
        const projectRoot = createTempRoot();
        writeProjectMetadata(projectRoot);
        coordinateDesktopIntegrationOpenMock.mockResolvedValue({
            provider,
        ...
        try {
            ...
            expect(coordinateDesktopIntegrationOpenMock).toHaveBeenCalledWith({ provider, action: 'prepare' }, expect.objectContaining({
    ...
    it('returns the desktop integration coordinator result without accepting launch details', async () => {
        ...
        try {
            ...
            expect(coordinateDesktopIntegrationOpenMock).toHaveBeenCalledWith({ provider: 'cursor', action: 'prepare' }, expect.objectContaining({
                inspect: expect.any(Function),
                launch: expect.any(Function),
                close: expect.any(Function),
                open: expect.any(Function),
    ...
    it('starts the selected local AI project without injecting Make when disabled in settings', async () => {
        ...
        try {
            ...
            expect(response.status).toBe(200);
            expect(coordinateDesktopIntegrationOpenMock).toHaveBeenCalledWith({ provider: 'cursor', action: 'normal' }, expect.any(Object));
            const adapters = coordinateDesktopIntegrationOpenMock.mock.calls[0]?.[1];
    ...
    it.each(['workbuddy', 'qoderwork'])('opens the %s project and embedded surface through one Agent Surface call', async (provider) => {
        const projectRoot = createTempRoot();
        writeProjectMetadata(projectRoot);
        coordinateDesktopIntegrationOpenMock.mockResolvedValue({
            provider,
    ...
    it('keeps unsupported desktop platforms on the existing normal open path', async () => {
        ...
        try {
            ...
            expect(openMakeAgentProjectOnlyMock).not.toHaveBeenCalled();
            expect(openMakeAgentSurfaceProjectMock).not.toHaveBeenCalled();
            expect(childProcessMock.spawn).toHaveBeenCalledWith('open', ['-a', 'Cursor', projectRoot], expect.objectContaining({ shell: false }));
        }
        finally {
            await server.close();
    ...
    it('detects local AI agent versions on demand without the config availability endpoint', async () => {
        ...
        runLocalCommandMock.mockImplementation(async (command, args) => {
            if (args.includes('--version')) {
                if (command === 'codex')
                    return { command, escapedCommand: 'codex --version', stdout: 'codex-cli 1.2.3\n', stderr: '' };
                if (command === 'claude')
                    return { command, escapedCommand: 'claude --version', stdout: 'Claude Code 2.3.4 (Claude Code)\n', stderr: '' };
                if (command === 'opencode')
                    throw Object.assign(new Error('command not found'), { code: 'ENOENT' });
                if (command === 'agent')
                    return { command, escapedCommand: 'agent --version', stdout: 'Cursor Agent 0.50.0\n', stderr: '' };
                if (command === 'qodercli')
                    return { command, escapedCommand: 'qodercli --version', stdout: 'qodercli 0.2.15\n', stderr: '' };
                if (command === 'codebuddy')
                    return { command, escapedCommand: 'codebuddy --version', stdout: 'CodeBuddy Code 2.44.0\n', stderr: '' };
                if (command === 'reasonix')
                    throw Object.assign(new Error('unknown flag: --version'), { code: 'EXIT_CODE' });
                if (command === 'grok')
                    return { command, escapedCommand: 'grok --version', stdout: 'grok 0.2.93\n', stderr: '' };
            }
        ...
        try {
            ...
            expect(runLocalCommandMock).not.toHaveBeenCalledWith(expect.stringMatching(/^(codex|claude|opencode)$/u), ['--version'], expect.any(Object));
    ...
    it('uses a configured CLI Agent command path when testing the saved local CLI Agent', async () => {
        ...
        try {
            ...
            expect(runLocalCommandMock).toHaveBeenCalledWith('C:\\Users\\demo\\AppData\\Roaming\\npm\\codex.cmd', ['--version'], expect.any(Object));
    ...
    it('opens a CLI agent terminal in the active project root', async () => {
        ...
        try {
            ...
            const firstSpawnCall = childProcessMock.spawn.mock.calls[0];
            expect(firstSpawnCall).toBeDefined();
            if (process.platform === 'darwin') {
                const spawnArgs = firstSpawnCall?.[1];
    ...
    it('opens Codex local app in the active project root', async () => {
        ...
        try {
            ...
            const spawnOptions = firstSpawnCall?.[2];
    ...
    it('uses the legacy Linux URL handler for OpenCode instead of Agent Surface injection', async () => {
        ...
        expect(childProcessMock.spawn).toHaveBeenCalledWith('xdg-open', ['opencode://open-project?directory=/workspace/Axhub%20Runtime'], expect.objectContaining({ cwd: directory, shell: false }));
    });
    ...
    it('launches a local application without accepting a project path', async () => {
        ...
        expect(childProcessMock.spawn).toHaveBeenCalledWith('open', ['-a', '/Applications/TRAE SOLO.app'], expect.objectContaining({ shell: false }));
    });
    it.each([
        {
            agent: 'qoderwork',
    ...
    ])('opens the detected $agent app bundle with the project directory on macOS', ({ agent, applicationPath, application }) => {
        const command = buildLocalAppOpenCommandForPlatform({
            agent: agent,
            directory: '/workspace/Axhub Runtime',
            platform: 'darwin',
    ...
    ])('launches the detected $agent executable with the project directory on Windows', ({ agent, applicationPath }) => {
    ...
    it('falls back to a Codex deeplink when direct Codex app launch fails', async () => {
        ...
        childProcessMock.spawn
            ...
            child.once.mockImplementation((event, callback) => {
                if (event === 'error') {
        ...
        const result = await openLocalAppAgent({
            ...
            availability: { status: 'installed', path: '/usr/local/bin/codex' },
        });
        expect(result).toMatchObject({
            success: true,
            agent: 'codex',
    ...
    it('opens CLI and web agents directly and reports unsupported web/CLI agent requests', async () => {
        const projectRoot = createTempRoot();
        await expect(openCLIAgent({
            agent: 'definitely-unsupported',
            targetPath: projectRoot,
        })).rejects.toThrow('Unsupported CLI agent');
        await expect(openCLIAgent({
            agent: 'gemini',
            targetPath: projectRoot,
            availability: { status: 'installed', path: '/usr/local/bin/gemini' },
        })).rejects.toThrow('Unsupported CLI agent');
        ...
        const web = await openWebAgent({
            agent: 'acp',
            targetPath: projectRoot,
            availability: { status: 'installed', path: '/usr/local/bin/npx' },
        });
    ...
    it.skip('starts OpenCode with its official web command without precreating a session', async () => {
        ...
        writeProjectMetadata(projectRoot);
        writeFile(path.join(adminRoot, 'opencode-webui/index.html'), [
            '<html><head>',
