...
describe('quick edit runtime script', () => {
    function createRuntimeHarness(extraWindow = {}, importedModules = {}) {
        const listeners = new Map();
        const messages = [];
        const appendedElements = [];
        const copiedPlainTexts = [];
        ...
        const createElementStub = (tagName) => {
            ...
            Object.defineProperty(element, 'textContent', {
                get() {
                    return ownTextContent + element.children.map((child) => child.textContent || '').join('');
                },
                set(value) {
        ...
        const windowStub = {
            ...
            sessionStorage: {
                getItem: vi.fn((key) => sessionValues.get(key) ?? null),
        ...
        const importModule = vi.fn(async (moduleUrl) => importedModules[moduleUrl]);
        windowStub.__importModule = importModule;
        const emitCopyListeners = (event) => {
        ...
        const runtimeScriptForTest = QUICK_EDIT_RUNTIME_SCRIPT.replace('axureExportModulePromise = import(moduleUrl).then', 'axureExportModulePromise = window.__importModule(moduleUrl).then');
    ...
    function expectStoredTransientRetryToken(windowStub, token) {
        expect(windowStub.sessionStorage.setItem).toHaveBeenCalledWith('__axhub_quick_edit_transient_vite_retry__', expect.any(String));
    ...
    it('loads Axure from the exact module URL supplied by the admin host', () => {
        ...
        const axureBranch = QUICK_EDIT_RUNTIME_SCRIPT.slice(QUICK_EDIT_RUNTIME_SCRIPT.indexOf('async function exportAxureJson(data)'), QUICK_EDIT_RUNTIME_SCRIPT.indexOf('async function captureScreenshot(data)'));
    ...
    it('persists ignored prototype runtime errors locally and suppresses matching reports', () => {
        ...
        expect(appendedElements).toHaveLength(0);
        expect(windowStub.localStorage.setItem).toHaveBeenCalledWith('__axhub_prototype_runtime_ignored_errors__', expect.any(String));
        reportHomeError();
        expect(appendedElements).toHaveLength(0);
    ...
    it('reloads once instead of reporting transient Vite html-proxy script failures', async () => {
        ...
        expectStoredTransientRetryToken(windowStub, '/prototypes/ref-app-home::html-proxy:/@id/__x00__/prototypes/ref-app-home/index.html?html-proxy&index=0.js');
        expect(appendedElements).toHaveLength(0);
    });
    it('reloads once instead of reporting transient preview loader script failures', async () => {
        ...
        expectStoredTransientRetryToken(windowStub, '/prototypes/ref-app-home::preview-loader:/prototypes/ref-app-home/__axhub-preview-loader.js');
        expect(appendedElements).toHaveLength(0);
    });
    ...
    it('recovers html-proxy failures when an old pathname-only retry token exists', async () => {
        ...
        const { appendedElements, emit, windowStub } = createRuntimeHarness({ fetch: fetchMock });
        windowStub.sessionStorage.setItem('__axhub_quick_edit_transient_vite_retry__', '/prototypes/ref-app-home');
        emit('window:error', {
            target: {
                tagName: 'SCRIPT',
                src: proxyUrl,
    ...
    it('reports the Vite transform error behind a repeated preview-loader failure', async () => {
        ...
        windowStub.sessionStorage.setItem('__axhub_quick_edit_transient_vite_retry__', JSON.stringify({
        ...
        await copyButton.addEventListener.mock.calls.find(([type]) => type === 'click')?.[1]();
