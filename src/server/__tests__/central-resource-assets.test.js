...
describe('central resource assets', () => {
    it('stores newly written canvas images and screenshots under the centralized asset root', async () => {
        ...
        try {
            ...
            const reviewDiagramUrl = scopeProjectApiUrl(projectRoot, `${server.origin}/api/canvas/resources/${encodeResourceCanvasPath('.assets/review/demo.html/diagrams/mermaid-1.excalidraw')}`);
    ...
    it('moves centralized assets when a resource file is moved in the sidebar', async () => {
        ...
        try {
            ...
            const board = current.tree.find((node) => node.itemKey === 'docs/board.excalidraw');
            expect(archive).toBeTruthy();
            expect(board).toBeTruthy();
            const nextTree = current.tree
                .filter((node) => node !== board)
                .map((node) => (node === archive ? { ...node, children: [...node.children, board] } : node));
    ...
    it('moves centralized assets when a resource file is renamed', async () => {
        ...
        const server = await startTestServer(projectRoot);
        try {
            const renamed = await fetch(scopeProjectApiUrl(projectRoot, `${server.origin}/api/docs/${encodeResourceCanvasPath('flows/原画布.excalidraw')}`), {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
