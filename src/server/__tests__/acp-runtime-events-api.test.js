...
describe('ACP runtime events API', () => {
    it('relays the runtime stream with server-owned project scope', async () => {
        ...
        try {
            const response = await fetch(`${make.origin}/api/acp/conversations/runtime/events?projectId=project-a&targetPath=prototypes%2Fhome&apiBaseUrl=http%3A%2F%2Fevil.invalid%2Fapi&workspacePath=%2Fevil&conversationStorePath=%2Fevil%2Fconversations.json`, { headers: { Accept: 'text/event-stream' } });
            ...
            expect(upstream.requests[0]?.searchParams.get('conversationStorePath')).toBe(`${projectRoot}/src/prototypes/home/.spec/acp/conversations.json`);
    ...
    it('relays durable conversation runtime with the same server-owned scope', async () => {
        ...
        try {
            const response = await fetch(`${make.origin}/api/acp/conversations/runtime/status?projectId=project-status&targetPath=prototypes%2Fhome&threadId=thread-1&workspacePath=%2Fevil&conversationStorePath=%2Fevil`);
            expect(response.status).toBe(200);
            expect(response.headers.get('access-control-allow-origin')).toBe('*');
            ...
            expect(request?.searchParams.get('conversationStorePath')).toBe(`${projectRoot}/src/prototypes/home/.spec/acp/conversations.json`);
        }
        finally {
            await make.close();
        }
