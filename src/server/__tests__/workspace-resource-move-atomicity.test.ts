...
function writeFilesystemDocsProject(projectRoot: string, id: string): string {
...
describe('workspace resource move atomicity', () => {
  it('preserves an unchanged existing folder basename exactly', async () => {
    ...
    try {
      const navigationUrl = scopeProjectApiUrl(
        projectRoot,
        `${server.origin}/api/workspace/navigation?tab=docs`,
      );
  ...
  it('lets a parent folder rename carry unchanged descendants', async () => {
    ...
    try {
      ...
      const current = await fetch(navigationUrl).then((response) => response.json());
      const nextTree = current.tree.map((node: Record<string, unknown>) => (
        node.folderPath === '父目录' ? { ...node, title: 'renamed parent' } : node
      ));
      const update = await fetch(navigationUrl, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  it('leaves every resource in place when a later move conflicts', async () => {
    ...
    try {
      ...
      const current = await fetch(navigationUrl).then((response) => response.json());
      const archive = current.tree.find((node: Record<string, unknown>) => node.folderPath === 'archive');
      const rootA = current.tree.find((node: Record<string, unknown>) => node.itemKey === 'docs/a.md');
      const rootB = current.tree.find((node: Record<string, unknown>) => node.itemKey === 'docs/b.md');
  ...
  it('renames a parent before moving another folder beneath it', async () => {
    ...
    try {
      ...
      const current = await fetch(navigationUrl).then((response) => response.json());
      const alpha = current.tree.find((node: Record<string, unknown>) => node.folderPath === 'alpha');
      const beta = current.tree.find((node: Record<string, unknown>) => node.folderPath === 'beta');
