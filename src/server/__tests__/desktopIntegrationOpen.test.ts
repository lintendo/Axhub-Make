...
import {
  ...
  type DesktopIntegrationOpenAdapters,
} from '../desktopIntegrationOpen.ts';

function inspection(
  overrides: Partial<DesktopIntegrationInspection> = {},
...
function createAdapters(
  overrides: Partial<DesktopIntegrationOpenAdapters> = {},
): DesktopIntegrationOpenAdapters {
  return {
...
describe('desktop integration open coordinator', () => {
  ...
  it('preserves a surface-only manual-directory notice', async () => {
    ...
    const adapters = createAdapters({
      open: vi.fn(async () => ({
        noticeCode: 'project-selection-required' as const,
        notice,
      })),
    });
  ...
  it('does not direct a missing integration to the removed installer command', async () => {
    const adapters = createAdapters({
      inspect: vi.fn(async () => inspection({
        integrationInstalled: false,
      })),
    ...
    await expect(coordinateDesktopIntegrationOpen({
      provider: 'chatgpt', action: 'prepare',
    }, adapters)).rejects.toThrow('Update @axhub/make and try again.');
    expect(adapters.launch).not.toHaveBeenCalled();
  });
