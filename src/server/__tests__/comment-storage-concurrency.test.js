...
function writeCommentProject(projectRoot) {
...
function scopedCommentUrl(projectRoot, origin, target) {
    return scopeProjectApiUrl(projectRoot, `${origin}${target.endpoint}`);
}
function pageScopeFor(index) {
    if (index === 0)
        return 'page-a';
    if (index === 1)
        return 'page-b';
...
function buildSnapshot(target, count, label, commentState = 'editing') {
...
function prepareJsonWrite(urlValue, payload) {
...
async function readPersistedDocument(projectRoot, url) {
    ...
    describe(`${target.name} concurrent persistence`, () => {
        it('retains every distinct record from 10, 12, and 15 item snapshots that finish newest first', async () => {
            ...
            try {
                ...
                for (const document of [responseDocument, fileDocument]) {
                    ...
                    expect(document.images.every((image) => (typeof image.assetPath === 'string' && !('data' in image)))).toBe(true);
                }
            }
            finally {
                await server.close();
        ...
        it('does not discard late active records when tombstone cleanup finishes after a larger save', async () => {
            ...
            try {
                ...
                expect(fileDocument.comments.some((comment) => comment.deletedAt)).toBe(false);
