...
import { normalizePrototypeCommentTargetPath, resolvePrototypeCommentStorage, } from './documentCommentsStorage.ts';
...
function resolvePrototypeCommentsPath(projectRoot, rawTargetPath, metadata) {
...
function sanitizeAssetBaseName(value, fallback) {
...
export function buildCommentIdentity(record) {
    return normalizeIdentityPart(record.id);
}
export function buildImageIdentity(record) {
    return normalizeIdentityPart(record.id);
}
export function normalizeObservedTombstones(value) {
    ...
    return value.flatMap((candidate) => {
        ...
        if (!commentId || !Number.isFinite(deletedAt) || deletedAt <= 0)
            return [];
        if (kind === 'comment') {
...
export function mergeStoredTombstones(previous, incoming) {
    if (!previous)
        return incoming;
    ...
    const commentTombstones = previousComments.filter((value) => isRecord(value) && isDeletedRecord(value));
    const commentBarriers = new Set(commentTombstones.map(buildCommentIdentity).filter(Boolean));
    const incomingActiveComments = incomingComments.filter((value) => {
        if (!isRecord(value))
            return true;
        return !commentBarriers.has(buildCommentIdentity(value));
    });
    const incomingCommentIdentities = new Set(incomingActiveComments
        .filter(isRecord)
        .map(buildCommentIdentity)
        .filter(Boolean));
    const preservedActiveComments = previousComments.filter((value) => {
        if (!isRecord(value) || isDeletedRecord(value))
            return false;
    ...
    const imageTombstones = previousImages.filter((value) => isRecord(value) && isDeletedRecord(value));
    ...
    const incomingImageIdentities = new Set(incomingActiveImages
        .filter(isRecord)
...
export function compactObservedTombstones(previous, observedTombstones) {
    ...
    const observedComments = observedTombstones.filter((value) => value.kind === 'comment');
    const matchedCommentIdentities = new Set();
    for (const value of comments) {
        ...
        if (!identity)
            continue;
        if (observedComments.some((tombstone) => (identity === tombstone.commentId
            && Number(value.deletedAt) === tombstone.deletedAt))) {
            matchedCommentIdentities.add(identity);
        }
    }
    const observedImages = observedTombstones.filter((value) => value.kind === 'image');
    const matchedImageIdentities = new Set();
    for (const value of images) {
        ...
        if (!identity)
            continue;
        if (observedImages.some((tombstone) => (identity === tombstone.id
            && Number(value.deletedAt) === tombstone.deletedAt))) {
...
function normalizeCommentDocument(input, resolved) {
...
function persistImageAssets(document, resolved) {
...
function normalizeAssetPath(rawValue, resolved) {
...
function collectImageAssetPaths(document, resolved) {
    const paths = new Set();
...
function resolveExistingImageAssetPath(assetPath, resolved) {
    ...
    if (!isPathInside(resolved.assetDir, fullPath) || !fs.existsSync(fullPath))
        return null;
    try {
...
function removeUnreferencedImageAssets(previous, next, resolved) {
    ...
    for (const assetPath of previousPaths) {
        if (nextPaths.has(assetPath))
            continue;
        const relativeAssetPath = assetPath.slice(`${resolved.projectRelativeAssetRoot}/`.length);
        const fullPath = path.resolve(resolved.assetDir, relativeAssetPath);
        if (!isPathInside(resolved.assetDir, fullPath))
            continue;
        try {
            if (!fs.existsSync(fullPath))
                continue;
            const realFullPath = fs.realpathSync(fullPath);
            if (!isPathInside(realAssetDir, realFullPath))
                continue;
...
function hydrateImageData(document, resolved, url) {
...
function mimeTypeFromFileName(filePath) {
...
function handleAssetRequest(req, res, context, url) {
    if (url.pathname !== '/api/prototype-comments/asset')
        return false;
    if (req.method !== 'GET') {
...
export function handlePrototypeCommentsApi(req, res, context, url) {
    if (handleAssetRequest(req, res, context, url))
        return true;
    if (url.pathname !== '/api/prototype-comments')
        return false;
    ...
    if (req.method === 'PUT') {
        readJsonBody(req)
            ...
            const observedTombstones = normalizeObservedTombstones(isRecord(body) ? body.observedTombstones : undefined);
            const merged = reason === 'restore' && previousDocument
                ? normalizeCommentDocument(compactObservedTombstones(previousDocument, observedTombstones), resolved)
                : reason === 'clear'
                    ? normalized
