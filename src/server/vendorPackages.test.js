...
import { buildVendorImportMap, createVendorAliases, loadVendorPackagesConfig, syncVendorPackages, withVendorSyncLock, } from '../../scripts/utils/vendor-packages.mjs';
...
function readAdminBundleJavaScript() {
...
describe('make-server vendor packages', () => {
    it('uses vendored packages from make-server config instead of workspace paths', () => {
        ...
        const agentSurfaceIntegration = fs.readFileSync(path.join(appRoot, 'src/server/agentSurfaceIntegration.ts'), 'utf8');
        expect(packageJson.dependencies).toMatchObject({
            '@axhub/agent-surface': 'file:vendor/agent-surface',
        ...
        expect(packageJson.pnpm?.overrides).not.toHaveProperty('tiptap-editor');
        ...
        expect(packageJson.files).toContain('vendor/agent-surface/dist/**');
        ...
        expect(packageJson.scripts?.['server:dev']).not.toContain('AXHUB_ONLINE_BASE_URL=');
        ...
        expect(tsconfig.compilerOptions.paths).toMatchObject({
            '@axhub/agent-surface': ['./vendor/agent-surface/dist/index.d.ts'],
        ...
        expect(agentSurfaceIntegration).toContain("from '../../vendor/agent-surface/dist/index.js'");
        expect(agentSurfaceIntegration).not.toContain("from '@axhub/agent-surface'");
    });
    it('loads server-owned vendor packages without client-only annotation runtime', () => {
        ...
        expect(aliases).toEqual(expect.arrayContaining([
            expect.objectContaining({
                packageName: '@axhub/agent-surface',
                runtimeEntryRelative: 'vendor/agent-surface/dist/index.js',
                typesEntryRelative: 'vendor/agent-surface/dist/index.d.ts',
            }),
            expect.objectContaining({
    ...
    it('keeps commentary annotation editor visibility host-configurable in vendored source and bundles', () => {
        const typesSource = fs.readFileSync(path.join(appRoot, 'vendor/axhub-commentary/src/web-editor-types.ts'), 'utf8');
        const stateSource = fs.readFileSync(path.join(appRoot, 'vendor/axhub-commentary/src/core/editor/state.ts'), 'utf8');
        const promptCardSource = fs.readFileSync(path.join(appRoot, 'vendor/axhub-commentary/src/ui/runtime/prompt-card-view.tsx'), 'utf8');
        const bundles = [
            fs.readFileSync(path.join(appRoot, 'vendor/axhub-commentary/dist/index.mjs'), 'utf8'),
            fs.readFileSync(path.join(appRoot, 'vendor/axhub-commentary/dist/index.js'), 'utf8'),
        ];
        expect(typesSource).toContain('showAnnotationMarkdownEditor?: boolean;');
        expect(stateSource).toContain('showAnnotationMarkdownEditor: options.host?.showAnnotationMarkdownEditor ?? true,');
        expect(promptCardSource).toContain('options.showAnnotationMarkdownEditor !== false');
        expect(promptCardSource).toContain('const annotationMarkdownEditorOpen = Boolean(');
        expect(promptCardSource).toContain('const showNoteComposer = !annotationMarkdownEditorOpen && !bubbleStyleEditorOpen;');
        expect(promptCardSource).toContain('const showPromptCardExecutionActions = !isAnnotationSession || !annotationMarkdownEditorOpen;');
        for (const bundle of bundles) {
            expect(bundle).toContain('showAnnotationMarkdownEditor: options.host?.showAnnotationMarkdownEditor ?? true');
            expect(bundle).toContain('options.showAnnotationMarkdownEditor !== false');
            expect(bundle).toContain('const annotationMarkdownEditorOpen = Boolean(');
            expect(bundle).toContain('const showNoteComposer = !annotationMarkdownEditorOpen && !bubbleStyleEditorOpen;');
            expect(bundle).toContain('const showPromptCardExecutionActions = !isAnnotationSession || !annotationMarkdownEditorOpen;');
        }
    });
    it('keeps vendored commentary from showing the AI note composer while annotation editing is open', () => {
        ...
        expect(source).toContain('const showNoteComposer = !annotationMarkdownEditorOpen && !bubbleStyleEditorOpen;');
        expect(source).toContain('{showNoteComposer ? (');
        expect(source).toContain('const showAnnotationMarkdownEditor = Boolean(');
        expect(source).toContain('annotationMarkdownEditorOpen && !bubbleStyleEditorOpen');
    ...
    it('keeps the vendored commentary annotation editor aligned with the runtime source', () => {
        ...
        for (const bundle of [esmBundle, cjsBundle]) {
            ...
            expect(bundle).toContain('annotationMarkdownEditorOpen && !bubbleStyleEditorOpen');
    ...
    it('keeps the installed commentary file dependency aligned with the vendored runtime bundle', () => {
        ...
        expect(installedBundle).toContain('const showNoteComposer = !annotationMarkdownEditorOpen && !bubbleStyleEditorOpen;');
        expect(installedBundle).toContain('annotationMarkdownEditorOpen && !bubbleStyleEditorOpen');
    ...
    it('keeps the vendored commentary durable-id tombstone contract in source and runtime bundles', () => {
        const typesSource = fs.readFileSync(path.join(appRoot, 'vendor/axhub-commentary/src/web-editor-types.ts'), 'utf8');
        const persistenceSource = fs.readFileSync(path.join(appRoot, 'vendor/axhub-commentary/src/core/editor/persistence.ts'), 'utf8');
        const lifecycleSource = fs.readFileSync(path.join(appRoot, 'vendor/axhub-commentary/src/core/editor/lifecycle.ts'), 'utf8');
        const esmBundle = fs.readFileSync(path.join(appRoot, 'vendor/axhub-commentary/dist/index.mjs'), 'utf8');
        const cjsBundle = fs.readFileSync(path.join(appRoot, 'vendor/axhub-commentary/dist/index.js'), 'utf8');
    ...
    it('syncs prebuilt package artifacts and writes generated metadata', () => {
        ...
        fs.writeFileSync(path.join(sourceRoot, 'package.json'), JSON.stringify({ name: 'demo-package', type: 'module' }, null, 2), 'utf8');
        const config = {
            packages: [
                {
        ...
        try {
            ...
            const generatedAliases = JSON.parse(fs.readFileSync(path.join(appTempRoot, 'vendor/vendor-aliases.generated.json'), 'utf8'));
            ...
            const generatedTsconfig = JSON.parse(fs.readFileSync(path.join(appTempRoot, 'vendor/vendor-tsconfig.generated.json'), 'utf8'));
    ...
    it('rewrites workspace dependencies between vendored packages to relative file specs', () => {
        ...
        fs.writeFileSync(path.join(dependencyRoot, 'package.json'), JSON.stringify({ name: 'dependency-package', type: 'module' }, null, 2), 'utf8');
        const config = {
            packages: [
                {
        ...
        try {
            ...
            const vendoredPackageJson = JSON.parse(fs.readFileSync(path.join(appTempRoot, 'vendor/consumer-package/package.json'), 'utf8'));
