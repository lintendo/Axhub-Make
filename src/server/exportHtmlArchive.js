...
function findNodeModuleFileFrom(startDir, relativePath) {
...
function resolveNodeModuleFile(projectRoot, relativePath) {
...
function toStaticFile(filePath, contentType, content) {
    return {
...
function normalizeDataUriBase64(value) {
    return value.replace(/\s+/gu, '');
}
function extractLargeCssDataUris(cssText) {
    const files = [];
    const dataUriPattern = /url\(\s*(["']?)(data:([^;,()]+)(?:;[^,()]*)?;base64,([A-Za-z0-9+/=\s]+))\1\s*\)/gu;
    const nextCssText = cssText.replace(dataUriPattern, (match, quote, _dataUri, contentType, base64) => {
...
function shouldExcludeSourceFile(relativePath) {
...
function collectSourceStaticFiles(projectRoot, sourceFile) {
    ...
    const sourceFiles = listFilesRecursive(sourceDir)
        ...
        .map((relativePath) => {
        const normalizedPath = relativePath.split(path.sep).join('/');
        return toStaticFile(`source/${normalizedPath}`, getContentType(normalizedPath), fs.readFileSync(path.join(sourceDir, relativePath)));
    });
...
function resolveMediaExportDir(projectRoot, mediaRoot) {
    const resolvedProjectRoot = path.resolve(projectRoot);
...
function toOfflineMediaPath(mediaPath) {
...
function rewriteMediaUrlsForOffline(text) {
    return text
        .replace(/(^|["'(\s=])\/api\/media\/file\/([^\s"'`)<>()]+)/gu, (_match, prefix, mediaPath) => (`${prefix}${toOfflineMediaPath(mediaPath)}`))
        .replace(/(^|["'(\s=])\/media\/([^\s"'`)<>()]+)/gu, (_match, prefix, mediaPath) => (`${prefix}${toOfflineMediaPath(mediaPath)}`));
}
function createReviewSubmitScript(reviewSubmit) {
...
export async function buildExportHtmlStaticFiles(options) {
...
export async function streamExportHtmlArchive(res, options, buildContentDisposition) {
