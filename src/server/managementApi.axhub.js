...
import { resolvePrototypeIdForReviewSubmit, } from './reviewLanSubmitConfig.ts';
import { AxhubApiError, createAxhubAuthClient, resolveRequestOrigin, } from './axhubAuthClient.ts';
...
function createAxhubContentHash(body) {
...
function isEmptyAxhubPublishFile(file) {
    return file.body.length <= 0;
}
function isAxhubCorePublishFilePath(filePath) {
    return AXHUB_CORE_PUBLISH_FILE_PATHS.has(filePath.split(path.sep).join('/'));
}
function assertNonEmptyAxhubCoreFiles(files) {
...
export function normalizeFilesForAxhub(files) {
...
function resolveAxhubHostedUrl(rawUrl, onlineBaseUrl) {
...
export function normalizeAxhubPublishResultUrl(result, onlineBaseUrl) {
    return {
        ...result,
        url: resolveAxhubHostedUrl(result.url, onlineBaseUrl),
    };
}
function sanitizeAxhubMe(me) {
    const record = me && typeof me === 'object' ? me : {};
    const { token: _token, accessToken: _accessToken, refreshToken: _refreshToken, ...safeRecord } = record;
    return {
        ...safeRecord,
...
export function handleAxhubApi(req, res, options, pathname, handlers) {
