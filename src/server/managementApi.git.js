...
const WORKSPACE_CHANGE_GROUP_ORDER = [
...
function resourceSourcePathForGit(resource) {
...
function normalizeGitTargetPath(context, rawTargetPath, handlers) {
...
function resolveGitVersionFilePath(projectRoot, versionId, requestedParts) {
...
function decodeGitVersionUrlPathParts(value) {
...
function parseGitPorcelainStatus(stdout) {
...
function parseWorkspaceVersionCommit(stdout) {
...
function parseWorkspaceVersionCommitList(stdout) {
...
function normalizeGitVersionRef(value) {
...
function displayNameFromResource(resource, fallback) {
    return String(resource?.title || resource?.displayName || resource?.name || resource?.id || fallback).trim() || fallback;
}
function getWorkspaceResourceSourceCandidates(resource) {
    return [
...
function findResourceBySourcePath(resources, filePath) {
    ...
    for (const resource of resources) {
        ...
        if (candidates.some((candidate) => (normalizedFile === candidate
...
function getPathSegmentName(filePath, marker) {
...
function getWorkspaceWriteTargetPath(metadata, type) {
...
function isPathInsideWorkspaceTarget(filePath, targetPath) {
    if (!targetPath)
        return false;
...
function pathContainsWorkspaceMarker(filePath, marker) {
    const normalizedFile = normalizeSlashPath(filePath);
...
function getWorkspaceMarkerForFile(filePath, markers) {
    return markers.find((marker) => pathContainsWorkspaceMarker(filePath, marker)) || '';
}
function classifyWorkspaceChangedFile(metadata, changedFile) {
    ...
    if (resourceMarker) {
        const fallback = getPathSegmentName(filePath, `${resourceMarker}/`);
        return {
            key: 'resources',
...
function createWorkspaceChangeSummary(metadata, changedFiles) {
    ...
    for (const changedFile of changedFiles) {
        ...
        const items = groupMap.get(classified.key) || new Map();
...
function filterWorkspaceChangedFilesByScope(changedFiles, scopePath) {
...
function readWorkspaceGitProjectConfig(projectRoot) {
...
function writeWorkspaceRemoteConfig(projectRoot, remote) {
    const configPath = getConfigPath(projectRoot);
    const current = readWorkspaceGitProjectConfig(projectRoot);
    const next = {
        ...current,
...
function getConfiguredWorkspaceRemote(projectRoot) {
...
async function resolveWorkspaceRemote(projectRoot, executor) {
...
async function configureWorkspaceRemote(projectRoot, remote, executor) {
...
function encodePreviewPathSegments(value) {
    return value
        .split('/')
        .filter(Boolean)
...
function normalizePreviewResourcePath(value) {
    return value
        .replace(/\\/g, '/')
        .replace(/^\/+/u, '')
        .replace(/\/+$/u, '');
...
function getPrototypeIndexGitPathCandidates(...paths) {
...
async function hasPrototypeAtCommit(projectRoot, commitHash, candidatePaths, executor) {
...
async function execGit(args, cwd, executor) {
    try {
        if (executor) {
        ...
        throw new Error(decodeCommandOutput(error?.stderr || error?.message || 'Git command failed').trim());
    }
}
...
function parseGitNullSeparatedOutput(output) {
    return output.toString('utf8').split('\0').filter(Boolean);
}
async function execGitBlob(args, cwd, executor) {
    try {
        if (executor) {
...
async function execGitBinary(args, cwd, executor) {
    try {
        if (executor) {
            const result = await executor('git', args, { cwd });
...
async function execWorkspaceCommand(command, args, cwd, executor) {
    try {
        if (executor) {
        ...
        throw new Error(decodeCommandOutput(error?.stderr || error?.message || `${command} command failed`).trim());
    }
}
async function probeGitAvailability(projectRoot, executor) {
    try {
...
async function getWorkspaceVersionCommit(projectRoot, ref, executor) {
    try {
...
async function getWorkspaceVersionChangedFiles(projectRoot, ref, executor) {
...
async function getWorkspaceVersionCommitList(projectRoot, range, executor, scopePath, maxCount) {
    try {
...
function createUnavailableRemoteComparison(reason, branch = '') {
    return {
        available: false,
...
async function getWorkspaceRemoteComparison(projectRoot, metadata, remote, localRef, remoteBranch, executor, scopePath) {
    if (!remote.url) {
...
async function getWorkspaceBranchOverview(projectRoot, executor) {
    let localBranches = [];
    let remoteBranches = [];
    try {
...
function isLikelyGitLabUrl(url) {
    return /(?:gitlab\.com|gitlab)/iu.test(url);
}
function parseRepositoryPathFromGitUrl(url) {
...
async function tryCreateRemoteRepository(params) {
    ...
    if (params.url && isLikelyGitHubUrl(params.url) && parsed) {
        ...
        await execWorkspaceCommand('gh', ['repo', 'create', `${parsed.owner}/${parsed.repo}`, visibilityFlag, '--source=.', '--remote=origin', '--confirm'], params.projectRoot, params.executor);
        return { created: true, mode: 'gh', message: '已通过 GitHub CLI 创建在线仓库' };
    }
    if (params.url && isLikelyGitLabUrl(params.url) && parsed) {
        const visibilityFlag = params.visibility === 'public' ? '--public' : '--private';
        await execWorkspaceCommand('glab', ['repo', 'create', `${parsed.owner}/${parsed.repo}`, visibilityFlag, '--remoteName', DEFAULT_REMOTE_NAME, '--yes'], params.projectRoot, params.executor);
        return { created: true, mode: 'glab', message: '已通过 GitLab CLI 创建在线仓库' };
    }
    if (repositoryName) {
        const visibilityFlag = params.visibility === 'public' ? '--public' : '--private';
        try {
            await execWorkspaceCommand('gh', ['repo', 'create', repositoryName, visibilityFlag, '--source=.', '--remote=origin', '--confirm'], params.projectRoot, params.executor);
            return { created: true, mode: 'gh', message: '已通过 GitHub CLI 创建在线仓库' };
        }
        catch (githubError) {
            try {
                await execWorkspaceCommand('glab', ['repo', 'create', repositoryName, visibilityFlag, '--remoteName', DEFAULT_REMOTE_NAME, '--yes'], params.projectRoot, params.executor);
                return { created: true, mode: 'glab', message: '已通过 GitLab CLI 创建在线仓库' };
            }
            catch (gitlabError) {
...
export function handleGitApi(req, res, options, pathname, url, handlers) {
    ...
    const executor = handlers.commandExecutor || options.gitWorkspaceCommandExecutor;
    const sendPathError = (rawPath) => {
        sendJson(res, {
    ...
    const getScopedGitTargetPath = (rawPath) => {
    ...
    if (pathname.startsWith('/api/git/workspace/')) {
        (async () => {
            ...
            if (pathname === '/api/git/workspace/status' && method === 'GET') {
                ...
                const remoteComparison = await getWorkspaceRemoteComparison(projectRoot, context.metadata, remote, 'HEAD', operationRemoteBranch, executor, scopedGitPath);
                let branchView;
                if (!requestedVersionRef) {
                    const hasExplicitLocalView = Boolean(requestedBranch || viewedBranch !== currentBranch);
                    const hasExplicitRemoteView = Boolean(hasExplicitLocalView
                    ...
                    const viewedRemoteComparison = !remote.url
                        ...
                        : !viewedRemoteBranch || !remoteBranchExists
                            ...
                            : hasExplicitRemoteView
                                ? await getWorkspaceRemoteComparison(projectRoot, context.metadata, remote, viewedBranch, viewedRemoteBranch, executor, scopedGitPath)
                                : remoteComparison;
                    branchView = {
                        branch: viewedBranch,
            ...
            if (pathname === '/api/git/workspace/commit' && method === 'POST') {
                ...
                let scopedGitPath = '';
                try {
                    scopedGitPath = getScopedGitTargetPath(body?.path);
                }
                catch {
                    sendPathError(String(body?.path || ''));
                    return;
                }
                if (!message) {
            ...
            if (pathname === '/api/git/workspace/remote' && method === 'POST') {
                ...
                const configuredRemote = await configureWorkspaceRemote(projectRoot, { url: remoteUrl, defaultBranch }, executor);
                sendJson(res, {
                    success: true,
                    projectId: context.project.id,
            ...
            if (pathname === '/api/git/workspace/prompt' && method === 'POST') {
                const scene = String(body?.scene || 'merge-required');
                const allowedScenes = [
                    'create-remote',
                    'auth-failed',
                    'merge-required',
        ...
        if (pathname === '/api/git/build-version' && req.method === 'POST') {
            ...
            try {
                ...
                for (const file of parseGitNullSeparatedOutput(fileList)) {
                    ...
                    if (!isPathInside(tempDir, targetFile))
                        continue;
