...
function resolveConfiguredAcpApiBaseUrl(config) {
...
async function relayAcpRuntimeEvents(params) {
    const upstreamUrl = new URL(`${params.apiBaseUrl.replace(/\/chat$/u, '')}/conversations/runtime/events`);
    ...
    try {
        ...
        params.res.setHeader('Content-Type', upstream.headers.get('content-type') || 'text/event-stream; charset=utf-8');
        ...
        try {
            while (true) {
                ...
                if (done || controller.signal.aborted || params.res.destroyed)
                    break;
...
async function relayAcpRuntimeStatus(params) {
    const upstreamUrl = new URL(`${params.apiBaseUrl.replace(/\/chat$/u, '')}/conversations/${encodeURIComponent(params.threadId)}/runtime`);
    upstreamUrl.searchParams.set('workspacePath', params.workspacePath);
    if (params.conversationStorePath) {
        upstreamUrl.searchParams.set('conversationStorePath', params.conversationStorePath);
    }
    ...
    try {
        ...
        if (controller.signal.aborted || params.res.destroyed)
            return;
        params.res.statusCode = upstream.status;
        params.res.setHeader('Content-Type', upstream.headers.get('content-type') || 'application/json; charset=utf-8');
...
export function handleAcpRuntimeEventsApi(req, res, options, context, pathname, getServerConfig) {
    if (pathname !== ACP_RUNTIME_EVENTS_PATH && pathname !== ACP_RUNTIME_STATUS_PATH)
        return false;
    ...
    const conversationStorePath = targetPath
        ? path.join(context.project.root, 'src', targetPath, '.spec', 'acp', 'conversations.json')
        : undefined;
