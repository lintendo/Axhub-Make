...
const DEFAULT_PLACEHOLDER_DIMENSIONS = { width: 1600, height: 900 };
...
function readImageDimensions(mimeType, buffer) {
...
function createImagePlaceholderDataUrl(dimensions) {
...
function replaceUriEncodedSvgImages(source) {
...
export function replaceEmbeddedImageAssets(source) {
    const withoutBase64Images = String(source || '').replace(BASE64_IMAGE_DATA_URL_PATTERN, (_match, mimeType, base64Payload) => {
 ...
 */
function sanitizeProcessEnv(code) {
 ...
 */
function loadVendorAliases(projectRoot) {
    try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const { loadVendorPackagesConfig, createVendorAliases } = require(path.resolve(projectRoot, 'scripts/utils/vendor-packages.mjs'));
...
async function importPackageFromProject(projectRoot, packageName) {
    let entryPath;
    try {
...
function getPackageExport(module, exportName, packageName) {
...
function getDefaultExport(module, packageName) {
...
function isAnnotationRuntimeModule(moduleId) {
 ...
 */
export async function buildOnDemand(projectRoot, entryFilePath, options = {}) {
    ...
    const viteBuild = getPackageExport(viteModule, 'build', 'vite');
    const react = getDefaultExport(reactModule, '@vitejs/plugin-react');
    const tailwindcss = getDefaultExport(tailwindcssModule, '@tailwindcss/vite');
    ...
    const outputBundle = outputs.find((item) => item && item.output && Array.isArray(item.output));
    ...
    const cssAsset = outputBundle?.output.find((item) => item.type === 'asset' &&
