...
import { resolveDocumentCommentStorage, } from './documentCommentsStorage.ts';
import { compactObservedTombstones, isRecord, mergeStoredTombstones, normalizeObservedTombstones, } from './managementApi.prototypeComments.ts';
function normalizeWriteReason(value) {
    return value === 'state' || value === 'restore' || value === 'clear' ? value : 'changes';
}
function readStoredDocument(filePath) {
...
function normalizeDocument(input, resolved) {
...
function assetFileName(id, index, extension) {
...
function persistImageAssets(document, resolved) {
    const images = (Array.isArray(document.images) ? document.images : []).map((rawImage, index) => {
        ...
        if (parsed) {
            ...
            if (!fullPath.startsWith(resolved.assetDir + path.sep))
                throw new Error('Invalid document comment asset path');
...
function normalizeAssetPath(value, resolved) {
    const raw = String(value ?? '').trim().replace(/\\/gu, '/').replace(/^\/+/, '');
    const expectedPrefix = `.axhub/make/comment-assets/${resolved.documentHash}/`;
    if (!raw.startsWith(expectedPrefix) || raw.includes('\0') || raw.split('/').some((segment) => segment === '.' || segment === '..'))
        return null;
    return raw;
}
function collectAssetPaths(document, resolved) {
...
function removeUnreferencedImageAssets(previous, next, resolved) {
    ...
    for (const assetPath of previousPaths) {
        ...
        if (!filePath.startsWith(resolved.assetDir + path.sep))
            continue;
        try {
            if (fs.existsSync(filePath))
...
function hydrateImages(document, resolved, url) {
    ...
    return {
        ...
        images: (Array.isArray(document.images) ? document.images : []).map((rawImage) => {
            ...
            if (!assetPath)
                return image;
            const filePath = path.resolve(resolved.commentFilePath, '..', '..', '..', '..', assetPath);
            if (!filePath.startsWith(resolved.assetDir + path.sep) || !fs.existsSync(filePath))
                return image;
...
function sendAsset(req, res, context, url) {
    if (url.pathname !== '/api/document-comments/asset')
        return false;
    if (req.method !== 'GET') {
...
export function handleDocumentCommentsApi(req, res, context, url) {
    if (sendAsset(req, res, context, url))
        return true;
    if (url.pathname !== '/api/document-comments')
        return false;
    const resolved = resolveDocumentCommentStorage(context.project.root, url.searchParams.get('path'));
    if (!resolved) {
    ...
    if (req.method === 'PUT') {
        readJsonBody(req).then((body) => {
            ...
            if (reason === 'restore' || reason === 'clear')
                removeUnreferencedImageAssets(previous, document, resolved);
