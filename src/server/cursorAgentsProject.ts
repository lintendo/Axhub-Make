...
function resolveCursorAppCandidates(context: CursorLauncherContext): {
...
function isCursorWorkbenchPage(target: CursorCdpTarget): boolean {
...
function cursorTargetKey(target: CursorCdpTarget): string | null {
  if (typeof target.id === 'string' && target.id) return `id:${target.id}`;
  if (typeof target.url !== 'string') return null;
  return `fallback:${String(target.title || '')}:${target.url}`;
}

function assertNoNewNonAgentsWorkbench(
  before: CursorCdpTarget[],
  after: CursorCdpTarget[],
): void {
  ...
  const incompatibleTarget = after.find((target) => {
    if (!isCursorWorkbenchPage(target) || isCursorWorkbenchTarget(target)) return false;
...
function cursorDesktopRouterPath(platform: 'darwin' | 'win32', appPath: string): string {
  return platform === 'darwin'
    ? path.posix.join(appPath, 'Contents/Resources/app/bin/cursor')
    : appPath;
}

async function ensureCursorDesktopRouter(
  platform: 'darwin' | 'win32',
  appPath: string,
  fileSystem: CursorLauncherFileSystem,
): Promise<string> {
  const routerPath = cursorDesktopRouterPath(platform, appPath);
  if (platform !== 'darwin') return routerPath;
  try {
...
async function waitForCursorAgentsTarget(options: {
  ...
  for (let attempt = 0; attempt < options.maxAttempts; attempt += 1) {
    try {
      if ((await options.probeTargets(CURSOR_DEBUG_PORT)).some(isCursorWorkbenchTarget)) return;
    } catch {
