...
import { analyzeFigmaMakeArtifact, buildFigmaMakeExportPrompt, buildFigmaMakeProbePayload, ensureFigmaMakeAiChat, ensureFigmaMakeMeta, runFigmaMakePackAndInspect, } from './exportMakeArtifacts.ts';
import { sendJson, sendText } from './http.ts';
function createAdapterRequiredPayload(context, targetPath, capability) {
    return {
...
function createBuildArtifactRequiredPayload(context, targetPath, details = {}) {
    return {
...
function createSourceMetadataRequiredPayload(context, targetPath, details = {}) {
    return {
 ...
 */
function buildComponentBridgeSnippet() {
    return `
...
export async function handleSourceBackedExports(req, res, options, pathname, url, handlers) {
    ...
    const { resource, runtimeBuiltJsPath, runtimeBuiltJsRelativePath, indexBundlePath, } = handlers.getAxureArtifactPaths(context, targetPath);
    if (pathname === '/api/export-index-bundle' && req.method === 'GET') {
        ...
        handlers.sendDisabledCapability(res, 424, (runtimeBuiltJsRelativePath
...
export function handleUnavailableManagement(req, res, pathname, sendDisabledCapability) {
