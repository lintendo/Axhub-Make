...
如果用户未明确需求，提供默认建议：提取设计主题、数据模型、项目文档。根据用户反馈调整执行计划。
...
**文件命名**：使用英文文件名，如 `users.json`、`orders.json`
...
**关键原则**：
- **文件名使用英文**：如 `users.json`、`orders.json`、`products.json`
- **tableName 使用中文**：如 "用户表"、"订单表"、"产品表"
...
**还原输出**：
- 生成符合 `development-standards.md` 和 `axure-api-guide.md` 规范的页面组件
- 页面还原完成后，需按开发规范验收
